...
void PushSedimentStack(void)
{
    //local variable declarations/definitions
    int isolid,                 //loop index for solids (particle) type
    ...
    double v1,                  //volume of layer 1 (before collapse) (m3)
    ...
    for (i = 1; i <= nlinks; i++)
    {
        ...
        for (j = 1; j <= nnodes[i]; j++)
        {
            ...
            if (vlayerch[i][j][ilayer] > maxvlayerch[i][j][ilayer])
            {
                ...
                if (nstackch[i][j] == maxstackch && stkchopt == 1
                    && maxstackch > 2)
                {
...
//mlv                                           i, j, simtime);
                    ...
                    hlayerch[i][j][1] =
                        (float) (combinedvolume / achbed[i][j][2]);

                    //set minimum layer volume after collapse (m3)
                    minvlayerch[i][j][1] =
                        (float) (minvolch * combinedvolume);

                    //set maximum layer volume after collapse (m3)
                    maxvlayerch[i][j][1] =
                        (float) (maxvolch * combinedvolume);
                    ...
                    for (isolid = 1; isolid <= nsolids; isolid++)
                    {
                        ...
                        csedch[isolid][i][j][1] =
                            (float) (combinedmass / combinedvolume);
                    ...
                    for (k = 3; k <= nstackch[i][j]; k++)
                    {
                        ...
                        for (isolid = 1; isolid <= nsolids; isolid++)
                        {
                            ...
                            tmass = (float) (csedch[isolid][i][j][k]
                        ...
                        if (ksim > 2)
                        {
                            ...
                            for (ichem = 1; ichem <= nchems; ichem++)
                            {
                                ...
                                tmass = (float) (cchemch[ichem][i][j][k]
                                                 * vlayerch[i][j][k] /
                                                 1000.0);
                ...
                if (nstackch[i][j] < maxstackch)
                {
                    ...
                    if (ilayer <= nstackch0[i][j])
                    {
                        ...
                        //set the standard thickness of the new surface layer (m)
                        hlayerch[i][j][ilayer] = hlayerch0[i][j][ilayer];
                        ...
                        elevlayerch[i][j][ilayer] =
                            elevlayerch[i][j][ilayer - 1] +
                            hlayerch[i][j][ilayer];
                        ...
                        bwlayerch[i][j][ilayer - 1] =
                            bwlayerch0[i][j][ilayer - 1];

                        //Restore original thickness of Layer ilayer-1 (m)
                        hlayerch[i][j][ilayer - 1] =
                            hlayerch0[i][j][ilayer - 1];
                    ...
                    else        //else nstackch >= nstackch0
                    {
                        ...
                        hlayerch[i][j][ilayer] = hlayerch0[i][j][ilayer0];
                    ...
                    for (isolid = 1; isolid <= nsolids; isolid++)
                    {
                        ...
                        csedch[isolid][i][j][ilayer] =
                            csedch[isolid][i][j][ilayer - 1];

                        //Compute solids mass transferred by burial (kg)
                        tmass = (float) (csedch[isolid][i][j][ilayer - 1]
                                         * vlayerch[i][j][ilayer -
                                                          1] / 1000.0);
                        ...
                        depsedchinmass[isolid][i][j][ilayer - 1] =
                            depsedchinmass[isolid][i][j][ilayer - 1] +
                            (float) (tmass);
                    ...
                    if (ksim > 2)
                    {
                        ...
                        for (ichem = 1; ichem <= nchems; ichem++)
                        {
                            ...
                            cchemch[ichem][i][j][ilayer] =
                                cchemch[ichem][i][j][ilayer - 1];

                            //Compute chemical mass transferred by burial (kg)
                            tmass = (float) (cchemch[ichem][i][j][ilayer - 1]
                                             * vlayerch[i][j][ilayer -
                                                              1] / 1000.0);
                            ...
                            depchemchinmass[ichem][i][j][ilayer - 1] =
                                depchemchinmass[ichem][i][j][ilayer - 1] +
                                (float) (tmass);
                    ...
                    if (hbank[i][j] <= 0.0)
                    {
                        ...
                        SimulationError (8, i, j, nstackch[i][j] + 1);
                    ...
                    sideslope[i][j] =
                        (float) (0.5 * (twidth[i][j] - bwidth[i][j]) /
                                 hbank[i][j]);
                }
                else            //else all stack elements full (nstackch = maxstackch)
                {
                    ...
                    if (stkchopt == 0)
                    {
                        ...
                        fprintf (echofile_fp,
                                 "\n\n\nSediment Stack Full:\n\n");
                        fprintf (echofile_fp,
                                 "  Exceeded maximum number of layers. Set\n");
                        fprintf (echofile_fp,
                                 "  stkchopt = 1 or increase maxstackch...\n\n");
                        fprintf (echofile_fp,
                                 "  link = %d  node = %d  time = %9.4f\n\n",
                                 i, j, simtime);
                        fprintf (echofile_fp,
                                 "  stkchopt = %d  maxstackch = %d\n",
                                 stkchopt, maxstackch);
                        ...
                        SimulationError (10, i, j, nstackch[i][j]);
                    }
                    else        //else nstackch = maxstackch (the stack is full)
                    {
                        ...
                        fprintf (echofile_fp,
                                 "  Exceeded maximum number of layers but\n");
                        fprintf (echofile_fp,
                                 "  stkchopt = 1 for stack collapse...\n\n");
                        fprintf (echofile_fp,
                                 "  link = %d  node = %d  time = %9.4f\n\n",
                                 i, j, simtime);
                        fprintf (echofile_fp,
                                 ...
                                 stkchopt, maxstackch);
                        fprintf (echofile_fp,
                                 "  Check PushSedimentStack for errors!!\n");
