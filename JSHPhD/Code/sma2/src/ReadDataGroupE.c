...
void ReadDataGroupE(void)
{
    //local variable declarations/definitions
    int iprop,                  //loop index for environmental property
    ...
    for (iprop = 1; iprop <= npropg; iprop++)
    {
        ...
        if (nenvgtf[iprop] > 0)
        {
            ...
            for (itf = 1; itf <= nenvgtf[iprop]; itf++)
            {
                ...
                fprintf (echofile_fp, "\n\n%s %d  %s %s\n",
                         "Function: ", tfid, "Description: ", tfname);
    ...
    if (ksim > 2)
    {
        ...
        for (iprop = 1; iprop <= npropov; iprop++)
        {
            ...
            if (pid != 8)
            {
                ...
                for (ilayer = maxstackov; ilayer >= 1; ilayer--)
                {
                    ...
                    ReadOverlandEnvironmentFile (iprop, ilayer, convunits,
                                                 scale);
            ...
            if (nenvovtf[iprop] > 0)
            {
                ...
                for (itf = 1; itf <= nenvovtf[iprop]; itf++)
                {
                    ...
                    for (k = 1; k <= nenvovtfpairs[iprop][itf]; k++)
                    {
                        ...
                        //Apply conversion and scale factors
                        envovtf[iprop][itf][k] =
                            envovtf[iprop][itf][k] * convunits * convtime *
                            scale;

                        //Echo function value and time break to file
                        fprintf (echofile_fp, "%18.3f  %15.3f\n",
                                 envovtf[iprop][itf][k],
                                 envovtftime[iprop][itf][k]);
        ...
        if (chnopt > 0)
        {
            ...
            if (npropch < 0)
            {
                ...
                fprintf (echofile_fp, "\n\n\nData Group Error:\n");
                fprintf (echofile_fp,
                         "  Number of properties must be >= 0\n");
                ...
                if (nenvchtf[iprop] > 0)
                {
                    ...
                    for (itf = 1; itf <= nenvchtf[iprop]; itf++)
                    {
                        ...
                        for (k = 1; k <= nenvchtfpairs[iprop][itf]; k++)
                        {
                            ...
                            //Apply conversion and scale factors
                            envchtf[iprop][itf][k] =
                                envchtf[iprop][itf][k] * convunits *
                                convtime * scale;

                            //Echo function value and time break to file
                            fprintf (echofile_fp, "%18.3f  %15.3f\n",
                                     envchtf[iprop][itf][k],
                                     envchtftime[iprop][itf][k]);
                ...
                if (nfpocovtf[isolid] > 0)
                {
                    ...
                    for (itf = 1; itf <= nfpocovtf[isolid]; itf++)
                    {
                        ...
                        for (k = 1; k <= nfpocovtfpairs[isolid][itf]; k++)
                        {
                            ...
                            //Apply conversion and scale factors
                            fpocovtf[isolid][itf][k] =
                                fpocovtf[isolid][itf][k] * convunits *
                                convtime * scale;

                            //Echo function value and time break to file
                            fprintf (echofile_fp, "%18.3f  %15.3f\n",
                                     fpocovtf[isolid][itf][k],
                                     fpocovtftime[isolid][itf][k]);
