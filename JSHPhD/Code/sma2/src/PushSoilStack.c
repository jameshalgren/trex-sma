...
void PushSoilStack(void)
{
    ...
    for (i = 1; i <= nrows; i++)
    {
        ...
        for (j = 1; j <= ncols; j++)
        {
            //if the cell is in the domain (imask not null)
            if (imask[i][j] != nodatavalue)
            {
                ...
                if (vlayerov[i][j][ilayer] > maxvlayerov[i][j][ilayer])
                {
                    ...
                    if (nstackov[i][j] == maxstackov && stkovopt == 1
                        && maxstackov >= 2)
                    {
...
//mlv                                                   i, j, simtime);
                        ...
                        hlayerov[i][j][1] =
                            (float) (combinedvolume / aovground[i][j]);

                        //set minimum layer volume after collapse (m3)
                        minvlayerov[i][j][1] =
                            (float) (minvolov * combinedvolume);

                        //set maximum layer volume after collapse (m3)
                        maxvlayerov[i][j][1] =
                            (float) (maxvolov * combinedvolume);
                        ...
                        for (k = 3; k <= nstackov[i][j]; k++)
                        {
                            ...
                            if (ksim > 2)
                            {
                                ...
                                for (ichem = 1; ichem <= nchems; ichem++)
                                {
                                    ...
                                    tmass = (float) (cchemov[ichem][i][j][k]
                                                     * vlayerov[i][j][k] /
                                                     1000.0);
                    ...
                    if (nstackov[i][j] < maxstackov)
                    {
                        ...
                        if (ilayer <= nstackov0[i][j])
                        {
                            ...
                            hlayerov[i][j][ilayer] = hlayerov0[i][j][ilayer];
                            ...
                            elevlayerov[i][j][ilayer] =
                                elevlayerov[i][j][ilayer - 1] +
                                hlayerov[i][j][ilayer];
                            ...
                            hlayerov[i][j][ilayer - 1] =
                                hlayerov0[i][j][ilayer - 1];
                        ...
                        else    //else nstackov > nstackov0
                        {
                            ...
                            hlayerov[i][j][ilayer] = hlayerov0[i][j][ilayer0];
                        ...
                        for (isolid = 1; isolid <= nsolids; isolid++)
                        {
                            ...
                            csedov[isolid][i][j][ilayer] =
                                csedov[isolid][i][j][ilayer - 1];

                            //Compute solids mass transferred by burial (kg)
                            tmass = (float) (csedov[isolid][i][j][ilayer - 1]
                                             * vlayerov[i][j][ilayer -
                                                              1] / 1000.0);
                            ...
                            depsedovinmass[isolid][i][j][ilayer - 1] =
                                depsedovinmass[isolid][i][j][ilayer - 1] +
                                (float) (tmass);
                        ...
                        if (ksim > 2)
                        {
                            ...
                            for (ichem = 1; ichem <= nchems; ichem++)
                            {
                                ...
                                cchemov[ichem][i][j][ilayer] =
                                    cchemov[ichem][i][j][ilayer - 1];

                                //Compute chemical mass transferred by burial (kg)
                                tmass =
                                    (float) (cchemov[ichem][i][j][ilayer - 1]
                                             * vlayerov[i][j][ilayer -
                                                              1] / 1000.0);
                                ...
                                depchemovinmass[ichem][i][j][ilayer - 1] =
                                    depchemovinmass[ichem][i][j][ilayer - 1] +
                                    (float) (tmass);
                    ...
                    else        //else all stack elements full (nstackov = maxstackov)
                    {
                        ...
                        if (stkovopt == 0)
                        {
                            ...
                            fprintf (echofile_fp,
                                     "  row = %d  col = %d  time = %9.4f\n\n",
                                     i, j, simtime);
                            fprintf (echofile_fp,
                                     "  stkovopt = %d  maxstackov = %d\n",
                                     stkovopt, maxstackov);
                            ...
                            SimulationError (9, i, j, nstackov[i][j] + 1);
                        }
                        else    //else stkovopt == 1
                        {
                            ...
                            fprintf (echofile_fp,
                                     ...
                                     stkovopt, maxstackov);
                            fprintf (echofile_fp,
                                     "  Check PushSoilStack for errors!!\n");
