...
void WriteSummarySolids(void)
{
    //local variable declarations
    int isolid;                 //loop index for solids (particle) type

    double *initialmass,        //initial mass in domain (m3) [solid]
    ...
    for (isolid = 1; isolid <= nsolids; isolid++)
    {
        ...
        initialmass[0] = initialmass[0] + initialsolidsmassov[isolid][0];

        //Compute final overland water column total solids mass at end (kg)
        finalmass[0] = finalmass[0] + finalsolidsmassov[isolid][0];
        ...
        if (chnopt > 0)
        {
            ...
            //compute initial overland water column total solids mass at start (kg)
            initialmass[0] = initialmass[0] + initialsolidsmassch[isolid][0];

            //Compute final overland water column total solids mass at end (kg)
            finalmass[0] = finalmass[0] + finalsolidsmassch[isolid][0];
        ...
        totalmassin[isolid] = totalmassin[isolid] + totalerssedinov[isolid];
        ...
        totalmassin[0] = totalmassin[0] + totalerssedinov[isolid];
        ...
        totalmassout[0] = totalmassout[0] + totaldepsedoutov[isolid];
        ...
        if (chnopt > 0)
        {
            ...
            totalmassin[0] = totalmassin[0] + totalerssedinch[isolid];
            ...
            totalmassout[0] = totalmassout[0] + totaldepsedoutch[isolid];
    ...
    for (isolid = 0; isolid <= nsolids; isolid++)
    {
        ...
        sourcemass[isolid] = initialmass[isolid] + totalmassin[isolid];

        //Compute sum of water column solids sink masses (kg)
        sinkmass[isolid] = finalmass[isolid] + totalmassout[isolid];
        ...
        if (isolid == 0)
        {
            ...
            fprintf (statsfile_fp,
                     "Total Solids (Sum of All Particle Types)\n\n");
        }
