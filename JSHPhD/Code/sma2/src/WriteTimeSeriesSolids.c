...
void WriteTimeSeriesSolids(void)
{
    ...
    float **sgroupsumov,        //reporting group sum for overland plane [igroup][ireport]
    ...
    if (nsedreports > 0)
    {
        ...
        for (igroup = 0; igroup <= nsgroups; igroup++)
        {
            ...
            if (sedexpfile_fp == NULL)
            {
                ...
                printf ("Can't open Sediment Export File:  %s\n",
                        sedexpfile[igroup]);
            ...
            for (j = 1; j <= nsedreports; j++)
            {
                ...
                if (sedunitsopt[j] == 2)
                {
                    ...
                    fprintf (sedexpfile_fp, ",%e",
                             sgroupsumov[igroup][j] * dischargeov *
                             sedconvert[j]);

                    //multiply output export by conversion factor
                    //and discharge and print to file
                    fprintf (sedexpfile_fp, ",%e",
                             sgroupsumch[igroup][j] * dischargech *
                             sedconvert[j]);
