...
void ReadDataGroupC(void)
{
    //local variable declarations/definitions
    int isolid,                 //loop index for solids type
    ...
    float gsdovtot;             //Overland grain size distribution total

    char fieldname[MAXNAMESIZE],        //name of field for chemical properties
    ...
    if (nsgroups < 1 || nsgroups > nsolids)
    {
        ...
        fprintf (echofile_fp, "\n\n\nData Group Error:\n");
        fprintf (echofile_fp,
                 "  Number of solids reporting groups must be >= 1\n");
    ...
    sgroupnumber = (int *) malloc ((nsgroups + 1) * sizeof (int));      //solids reporting group number
    ...
    for (i = 1; i <= nsolids; i++)
    {
        ...
        if (depovopt > 1 || depchopt > 1 || ersovopt > 1 || erschopt > 1)
        {
            ...
            if (depovopt > 1)
            {
                ...
                if (tcdov[i] <= 0.0)
                {
                    ...
                    fprintf (echofile_fp,
                             "  Tau Critical for Overland Deposition must be > 0.0\n");
            ...
            if (chnopt > 0)
            {
                ...
                if (depchopt > 1)
                {
                    ...
                    if (tcdch[i] <= 0.0)
                    {
                        ...
                        fprintf (echofile_fp,
                                 "\n\n\nParticle Propery Error:\n");
                        fprintf (echofile_fp,
                                 "  Tau Critical for Channel Deposition must be > 0.0\n");
                ...
                if (erschopt == 1)
                {
                    ...
                    //Echo particle characteristics to file
                    fprintf (echofile_fp, "  %13.8f", vcch[i]);
                }
        ...
        //Echo particle characteristics to file
        fprintf (echofile_fp, "  %15d  %s", sgroupnumber[i], particlename[i]);
...
*/
                    ...
                    else
                    {
                        ...
                        fprintf (echofile_fp, "\n\n\nData Group Error:\n");
                        fprintf (echofile_fp,
                                 "  Undefined solids reaction constant\n");
                        fprintf (echofile_fp, "  User selected sid = %d\n",
                                 sid);
                        fprintf (echofile_fp,
                                 "  No constant defined for this identifier");
    ...
    for (i = 1; i <= nsoils; i++)
    {
        ...
        if (infopt == 0)
        {
            ...
            if (ersovopt <= 1)
            {
                ...
                fprintf (echofile_fp, "%4d  %17.5f  %15.7f", i, kusle[i],
                         vcov[i]);
            }
            else                //else ersovopt > 1
            {
        ...
        else                    //if there is infiltration (may need elseif when more options are added)
        {
            ...
            fprintf (echofile_fp, "  %24.5f  %8.6f  %25.5f  %25.5f  %s\n",
                     porosityov[i], khsoil[i], capshsoil[i], soilmd[i],
                     soilname[i]);
    ...
    if (ersovopt <= 1)
    {
        ...
        fprintf (echofile_fp, "\nManning n");
        fprintf (echofile_fp, "  Interception Depth (m)");
        ...
        for (i = 1; i <= nlands; i++)
        {
            ...
            fprintf (echofile_fp, "%9.5f  %22.3f  %5.3f  %5.3f %s",
                     nmanningov[i],
                     interceptionclass[i], cusle[i], pusle[i], landname[i]);
    ...
    else                        //else ersovopt > 1
    {
        ...
        for (i = 1; i <= nlands; i++)
        {
            ...
            fprintf (echofile_fp, "%9.5f  %22.3f  %11.3f  %s",
                     nmanningov[i],
                     interceptionclass[i], ayov[i], landname[i]);
        ...
        for (i = 1; i <= nrows; i++)
        {
            ...
            for (j = 1; j <= ncols; j++)
            {
                ...
                if (imask[i][j] > 0)
                {
                    ...
                    csedov[isolid][i][j] =
                        (float *) malloc ((maxstackov + 1) * sizeof (float));
    ...
    if (chnopt > 0)
    {
        ...
        if (maxstackch <= 2)
        {
            ...
            fprintf (echofile_fp,
                     "  Minimum value for maxstackch must be > 2\n");
            fprintf (echofile_fp,
                     "    Value entered for maxstackch = %d \n\n",
                     maxstackch);
    ...
    //Write label for overland solids loads/forcing functions
    fprintf (echofile_fp,
             "\n\n\n  Overland Solids Loads/Forcing Fucntions  \n");
    fprintf (echofile_fp, "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n");

    //Allocate memory for overland solids loads/forcing functions (w)
    nswov = (int *) malloc ((nsolids + 1) * sizeof (int));      //allocate memory
    swovrow = (int **) malloc ((nsolids + 1) * sizeof (int *)); //allocate memory
    swovcol = (int **) malloc ((nsolids + 1) * sizeof (int *)); //allocate memory
    nswovpairs = (int **) malloc ((nsolids + 1) * sizeof (int *));      //allocate memory
    swovopt = (int **) malloc ((nsolids + 1) * sizeof (int *)); //allocate memory
    swov = (float ***) malloc ((nsolids + 1) * sizeof (float **));      //allocate memory
    swovtime = (float ***) malloc ((nsolids + 1) * sizeof (float **));  //allocate memory
    ...
    for (i = 1; i <= nsolids; i++)
    {
        ...
        fscanf (inputfile_fp, "%s %d",  //read
                ...
                &nswov[i]);     //number of overland solid loads for current solids type

        //Echo number of loads and description to file
        fprintf (echofile_fp, "\n\n %s %d  %s %d  %s %s\n",
                 "Solids Type: ", i, "Number of Loads: ", nswov[i],
        ...
        if (nswov[i] > 0)
        {
            ...
            //Allocate additional memory for overland solids loads/forcing functions
            swovrow[i] = (int *) malloc ((nswov[i] + 1) * sizeof (int));        //allocate memory
            swovcol[i] = (int *) malloc ((nswov[i] + 1) * sizeof (int));        //allocate memory
            nswovpairs[i] = (int *) malloc ((nswov[i] + 1) * sizeof (int));     //allocate memory
            swovopt[i] = (int *) malloc ((nswov[i] + 1) * sizeof (int));        //allocate memory
            swov[i] = (float **) malloc ((nswov[i] + 1) * sizeof (float *));    //allocate memory
            swovtime[i] = (float **) malloc ((nswov[i] + 1) * sizeof (float *));        //allocate memory
            ...
            for (j = 1; j <= nswov[i]; j++)
            {
                ...
                fscanf (inputfile_fp, "%d %d %d %d",    //read
                        &swovrow[i][j], //overland soilds load row
                        &swovcol[i][j], //overland soilds load column
                        &nswovpairs[i][j],      //number of pairs (time breaks) in function
                        &swovopt[i][j]);        //overland solids load option
                ...
                fprintf (echofile_fp,
                         ...
                         "  Load Number: ", j, "  Row Number: ",
                         swovrow[i][j], "  Column Number: ", swovcol[i][j],
                         "  Number of Time Breaks: ", nswovpairs[i][j],
                         "  Load Option: ", swovopt[i][j], "  Description: ",
                         loadname);

                //Allocate remaining memory for solids loads/forcing functions
                swov[i][j] = (float *) malloc ((nswovpairs[i][j] + 1) * sizeof (float));        //allocate memory
                swovtime[i][j] = (float *) malloc ((nswovpairs[i][j] + 1) * sizeof (float));    //allocate memory
                ...
                if (swovopt[i][j] == 1)
                {
                    fprintf (echofile_fp, "  Concentration (g/m3)     ");       //load in g/m3 (concentration) when swovopt = 1
                ...
                {
                    fprintf (echofile_fp, "  Load (kg/day)     ");      //load in kg/day when swovopt = 0
                    fprintf (echofile_fp, "  Time (days)  \n");
                    fprintf (echofile_fp, "~~~~~~~~~~~~~~~~~   ");
                    fprintf (echofile_fp, "~~~~~~~~~~~~~~~\n");
                }
                ...
                for (k = 1; k <= nswovpairs[i][j]; k++)
                {
                    //Record 41
                    fscanf (inputfile_fp, "%f %f",      //read
                            &swov[i][j][k],     //Solid Load value
                            &swovtime[i][j][k]);        //Solid load time break

                    //Apply conversion and scale factors
                    swov[i][j][k] =
                        swov[i][j][k] * convunits * convtime * scale;

                    //Echo load value and time break to file
                    fprintf (echofile_fp, "%16.3f %19.3f\n",
                             swov[i][j][k], swovtime[i][j][k]);
    ...
    if (chnopt > 0)
    {
        ...
        fprintf (echofile_fp,
                 "\n\n\n  Solids Loads/Forcing Fucntions (Channels)  \n");
        fprintf (echofile_fp,
                 "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n");
        ...
        for (i = 1; i <= nsolids; i++)
        {
            ...
            //Echo number of loads and description to file
            fprintf (echofile_fp, "\n\n %s %d  %s %d  %s %s\n",
                     "Solids Type:", i, "Number of Loads:", nswch[i],
                     "Description:", loadname);
            ...
            if (nswch[i] > 0)
            {
                ...
                for (j = 1; j <= nswch[i]; j++)
                {
                    ...
                    fprintf (echofile_fp,
                             "\n\n %s %5d %s %5d %s %5d %s %5d %s %5d %s %s\n\n",
                             "  Load Number: ", j, "  Link Number: ",
                             swchlink[i][j], "  Node Number: ",
                             swchnode[i][j], "  Number of Time Breaks: ",
                             nswchpairs[i][j], "  Load Option: ",
                             swchopt[i][j], "  Description: ", loadname);
                    ...
                    for (k = 1; k <= nswchpairs[i][j]; k++)
                    {
                        ...
                        //Apply conversion and scale factors
                        swch[i][j][k] =
                            swch[i][j][k] * convunits * convtime * scale;
    ...
    for (i = 1; i <= noutlets; i++)
    {
        ...
        if (dbcopt[i] > 0)
        {
            ...
            {
                //Record 48 (Part 1)
                fscanf (inputfile_fp, "%d", &nsbcpairs[i][j]);  //Number of pairs for this solids boundary conditions
                ...
                fprintf (echofile_fp,
                         "\n\n%s %5d %s %5d %s %5d %s %5d %s %5d %s %s\n\n",
                         "  Outlet: ", i, "  Row Number: ", iout[i],
                         "  Column Number: ", jout[i], "  Solids Type: ", j,
                ...
                for (k = 1; k <= nsbcpairs[i][j]; k++)
                {
                    ...
                    //Apply conversion and scale factors
                    sbc[i][j][k] =
                        sbc[i][j][k] * convunits * convtime * scale;
    ...
    for (i = 1; i <= nsedreports; i++)
    {
        ...
        fprintf (echofile_fp, "%11d   %12d   %15d   %23.3f   %23d %s\n",
                 i, sedreprow[i], sedrepcol[i], sedarea[i], sedunitsopt[i],
                 varname);
