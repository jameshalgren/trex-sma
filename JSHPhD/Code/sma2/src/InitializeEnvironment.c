...
void InitializeEnvironment(void)
{
    ...
    int iprop;                  //index for number of propertis specified
    ...
    for (iprop = 1; iprop <= npropg; iprop++)
    {
        ...
        nenvgtft[iprop] = (float *) malloc ((nenvgtf[i] + 1) * sizeof (float)); //next (upper) interval time for general property time series
        penvgtft[iprop] = (float *) malloc ((nenvgtf[i] + 1) * sizeof (float)); //prior (lower) interval time for general property time series
        benvgtf[iprop] = (float *) malloc ((nenvgtf[i] + 1) * sizeof (float));  //intercept for general property interpolation
        menvgtf[iprop] = (float *) malloc ((nenvgtf[i] + 1) * sizeof (float));  //slope for general property interpolation
        envgtfpoint[iprop] = (int *) malloc ((nenvgtf[i] + 1) * sizeof (int));  //pointer (to position in time series) for general function
        envgtfinterp[iprop] = (float *) malloc ((nenvgtf[i] + 1) * sizeof (float));     //general property function interpolated in time
    ...
    for (iprop = 1; iprop <= npropov; iprop++)
    {
        ...
        nenvovtft[iprop] = (float *) malloc ((nenvovtf[i] + 1) * sizeof (float));       //next (upper) interval time for overland property time series
        penvovtft[iprop] = (float *) malloc ((nenvovtf[i] + 1) * sizeof (float));       //prior (lower) interval time for overland property time series
        benvovtf[iprop] = (float *) malloc ((nenvovtf[i] + 1) * sizeof (float));        //intercept for overland property interpolation
        menvovtf[iprop] = (float *) malloc ((nenvovtf[i] + 1) * sizeof (float));        //slope for overland property interpolation
        envovtfpoint[iprop] = (int *) malloc ((nenvovtf[i] + 1) * sizeof (int));        //pointer (to position in time series) for overland function
        envovtfinterp[iprop] = (float *) malloc ((nenvovtf[i] + 1) * sizeof (float));   //overland property function interpolated in time
    ...
    if (chnopt > 0)
    {
        ...
        for (iprop = 1; iprop <= npropch; iprop++)
        {
            ...
            nenvchtft[iprop] = (float *) malloc ((nenvchtf[i] + 1) * sizeof (float));   //next (upper) interval time for channel property time series
            penvchtft[iprop] = (float *) malloc ((nenvchtf[i] + 1) * sizeof (float));   //prior (lower) interval time for channel property time series
            benvchtf[iprop] = (float *) malloc ((nenvchtf[i] + 1) * sizeof (float));    //intercept for channel property interpolation
            menvchtf[iprop] = (float *) malloc ((nenvchtf[i] + 1) * sizeof (float));    //slope for channel property interpolation
            envchtfpoint[iprop] = (int *) malloc ((nenvchtf[i] + 1) * sizeof (int));    //pointer (to position in time series) for channel function
            envchtfinterp[iprop] = (float *) malloc ((nenvchtf[i] + 1) * sizeof (float));       //channel property function interpolated in time
    ...
    watertempov = (float ***) malloc ((nrows + 1) * sizeof (float **)); //overland water temperature (C) [row][col][layer]
    ...
    for (i = 1; i <= nrows; i++)
    {
        ...
        watertempov[i] = (float **) malloc ((ncols + 1) * sizeof (float *));    //overland water temperature (C) [row][col][layer]
        ...
        for (j = 1; j <= ncols; j++)
        {
            ...
            watertempov[i][j] = (float *) malloc ((maxstackov + 1) * sizeof (float));   //overland water temperature (C) [row][col][layer]
            ...
            {
                //set default values to zero (fdoc = 1.0, pH = 7.0)
                ...
                watertempov[i][j][k] = 0.0;     //overland water temperature (C) [row][col][layer]
    ...
    if (chnopt > 0)
    {
        ...
        watertempch = (float ***) malloc ((nlinks + 1) * sizeof (float **));    //channel water temperature (C) [link][node][layer]
        ...
        for (i = 1; i <= nlinks; i++)
        {
            ...
            watertempch[i] = (float **) malloc ((nnodes[i] + 1) * sizeof (float *));    //channel water temperature (C) [link][node][layer]
            ...
            for (j = 1; j <= nnodes[i]; j++)
            {
                ...
                watertempch[i][j] = (float *) malloc ((maxstackch + 1) * sizeof (float));       //channel water temperature (C) [link][node][layer]
                ...
                for (k = 0; k <= maxstackch; k++)
                {
                    ...
                    watertempch[i][j][k] = 0.0; //channel water temperature (C) [link][node][layer]
