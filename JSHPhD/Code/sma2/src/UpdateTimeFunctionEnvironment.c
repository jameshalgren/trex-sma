...
void UpdateTimeFunctionEnvironment(void)
{
    //local variable declarations/definitions
    int iprop,                  //loop index for properties
    ...
    for (iprop = 1; iprop <= npropg; iprop++)
    {
        ...
        for (j = 1; j <= nenvgtf[i]; j++)
        {
            ...
            envgtfinterp[i][j] =
                (float) (menvgtf[i][j] * (simtime - nenvgtft[i][j]) +
                         benvgtf[i][j]);
    ...
    for (iprop = 1; iprop <= npropov; iprop++)
    {
        ...
        if (simtime >= ntenvovtf[i])
        {
            ...
            for (j = 1; j <= nenvovtf[i]; j++)
            {
                ...
                if (simtime >= nenvovtft[i][j] || simtime < penvovtft[i][j])
                {
                    ...
                    menvovtf[i][j] =
                        (envovtf[i][j][ip] -
                         envovtf[i][j][ip + 1]) / (envovtftime[i][j][ip] -
                                                   envovtftime[i][j][ip + 1]);
                    ...
                    //Next time interval upper bound to recalculate this function
                    nenvovtft[i][j] =
                        (float) (ncycle * endtime) + envovtftime[i][j][ip +
                                                                       1];
        ...
        for (j = 1; j <= nenvovtf[i]; j++)
        {
            //Compute linearly interpolated values for this time step
            envovtfinterp[i][j] =
                (float) (menvovtf[i][j] * (simtime - nenvovtft[i][j]) +
                         benvovtf[i][j]);
        ...
        for (iprop = 1; iprop <= npropch; iprop++)
        {
            ...
            if (simtime >= ntenvchtf[i])
            {
                ...
                for (j = 1; j <= nenvchtf[i]; j++)
                {
                    ...
                    //
                    if (simtime >= nenvchtft[i][j]
                        || simtime < penvchtft[i][j])
                    {
                        //If the simulation time is greater than the last time specified,
                        //start again (cyclically) with the first value in the array.  So
                        //work with the current time, modulo the last entry in the array.
                        ...
                        menvchtf[i][j] =
                            (envchtf[i][j][ip] -
                             envchtf[i][j][ip + 1]) / (envchtftime[i][j][ip] -
                                                       envchtftime[i][j][ip +
                                                                         1]);
                        ...
                        //Next time interval upper bound to recalculate this function
                        nenvchtft[i][j] =
                            (float) (ncycle * endtime) +
                            envchtftime[i][j][ip + 1];
            ...
            for (j = 1; j <= nenvchtf[i]; j++)
            {
                //Compute linearly interpolated values for this time step
                envchtfinterp[i][j] =
                    (float) (menvchtf[i][j] * (simtime - nenvchtft[i][j]) +
                             benvchtf[i][j]);
        ...
        for (i = 1; i <= nsolids; i++)
        {
            ...
            if (simtime >= ntfpocovtf[i])
            {
                ...
                for (j = 1; j <= nfpocovtf[i]; j++)
                {
                    ...
                    //
                    if (simtime >= nfpocovtft[i][j]
                        || simtime < pfpocovtft[i][j])
                    {
                        //If the simulation time is greater than the last time specified,
                        //start again (cyclically) with the first value in the array.  So
                        //work with the current time, modulo the last entry in the array.
                    ...
                    //Get the necessary time window bounds to update any function
                    if (nfpocovtft[i][j] < nt)
                        nt = nfpocovtft[i][j];
                    if (pfpocovtft[i][j] > pt)
                        pt = pfpocovtft[i][j];
            ...
            for (j = 1; j <= nfpocovtf[i]; j++)
            {
                //Compute linearly interpolated values for this time step
                fpocovtfinterp[i][j] =
                    (float) (mfpocovtf[i][j] * (simtime - nfpocovtft[i][j]) +
                             bfpocovtf[i][j]);
            ...
            for (i = 1; i <= nsolids; i++)
            {
                ...
                if (simtime >= ntfpocchtf[i])
                {
                    ...
                    for (j = 1; j <= nfpocchtf[i]; j++)
                    {
                        ...
                        //
                        if (simtime >= nfpocchtft[i][j]
                            || simtime < pfpocchtft[i][j])
                        {
                            //If the simulation time is greater than the last time specified,
                            //start again (cyclically) with the first value in the array.  So
                            //work with the current time, modulo the last entry in the array.
                            endtime =
                                fpocchtftime[i][j][nfpocchtfpairs[i][j]];
                            ...
                            while ((mtime < fpocchtftime[i][j][ip])
                                   && (ip != 1))
                            {
                                ip = ip - 1;
                        ...
                        //Get the necessary time window bounds to update any function
                        if (nfpocchtft[i][j] < nt)
                            nt = nfpocchtft[i][j];
                        if (pfpocchtft[i][j] > pt)
                            pt = pfpocchtft[i][j];
