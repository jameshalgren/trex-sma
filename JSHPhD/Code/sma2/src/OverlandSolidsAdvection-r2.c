...
void OverlandSolidsAdvection(void)
{
    ...
    float cin[11],              //sediment concentration of inflowing cell/node (source 0-10)
      load,                     //load (source or sink) (g/s)
      outflux;                  //overland solids outflux (g/s) (sum for sources 0, 1, 3, 5, and 7)

    double potential,           //solids mass transport potential (g)
        ...
        for (j = 1; j <= ncols; j++)
        {
            ...
            if (imask[i][j] != nodatavalue)
            {
                ...
                for (isolid = 1; isolid <= nsolids; isolid++)
                {
                    ...
                    for (k = 1; k <= nswov[isolid]; k++)
                    {
                        //if the current cell has a point source (row/col match)
                        if (swovrow[isolid][k] == i
                            && swovcol[isolid][k] == j)
                        {
                            ...
                            if (load >= 0.0)
                            {
                                ...
                                advsedovinflux[isolid][i][j][0] = load;
                            ...
                            else        //else the load is negative (a sink)
                            {
                                ...
                                advsedovoutflux[isolid][i][j][0] = load;

                            }   //end if load >= 0.0

                        }       //end if swovrow = i and swovcol = j
                    ...
                    for (k = 1; k <= 7; k = k + 2)
                    {
                        ...
                        advsedovinflux[isolid][i][j][k] =
                            advinflowov[i][j][k] * cin[k];

                        //compute the outflowing sediment flux (g/s)
                        advsedovoutflux[isolid][i][j][k] =
                            advoutflowov[i][j][k] * csedov[isolid][i][j][0];

                    }           //end loop over overland sources

                    //Determine if outflow potential exceeds available mass...
                    //
                    //Compute the soilds advective outflow potential...
                    //
                    //initialize the outflow potential
                    potential = 0.0;

                    //Note:  We actually only need to loop over sources
                    //       0, 1, 3, 5, and 7.
                    //
                    ...
                    {
                        //scale the soilds advection outflux (g/s) (source 0)
    ...
    for (i = 1; i <= noutlets; i++)
    {
        ...
        for (isolid = 1; isolid <= nsolids; isolid++)
        {
            ...
            potential =
                advsedovoutflux[isolid][iout[i]][jout[i]][10] * dt[idt];
