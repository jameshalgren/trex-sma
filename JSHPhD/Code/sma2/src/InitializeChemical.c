...
void InitializeChemical(void)
{
    //Local variable declarations/definitions
    int ichem,                  //loop index for number of chemical types
      isolid,                   //loop index for number of solids types
      nsources;                 //number of transport sources/directions

    float tbs;                  //total bed solids concentration (g/m3)
    ...
    for (ichem = 1; ichem <= nchems; ichem++)
    {
        ...
        for (i = 1; i <= nrows; i++)
        {
            ...
            for (j = 1; j <= ncols; j++)
            {
                //if the cell is i the domain (imask != nodatavalue)
    ...
    totalinfchemoutov = (double *) malloc ((nchems + 1) * sizeof (double));     //cumulative gross chemical mass transported out of the overland water column by infiltration (kg)
    ...
    if (chnopt == 1)
    {
        ...
        totalinfchemoutch = (double *) malloc ((nchems + 1) * sizeof (double)); //cumulative gross chemical mass transported out of the channel water column by infiltration (kg)
    ...
    ncwovt = (float **) malloc ((nchems + 1) * sizeof (float *));       //next (upper) interval time for chemical load time series
    pcwovt = (float **) malloc ((nchems + 1) * sizeof (float *));       //prior (lower) interval time for chemical load time series
    bcwov = (float **) malloc ((nchems + 1) * sizeof (float *));        //intercept for chemical load interpolation
    mcwov = (float **) malloc ((nchems + 1) * sizeof (float *));        //slope for chemical load interpolation
    cwovpoint = (int **) malloc ((nchems + 1) * sizeof (int *));        //pointer (to position in time series) for chemical loads
    cwovinterp = (float **) malloc ((nchems + 1) * sizeof (float *));   //external chemical load interpolated in time for source[i]
    ntcwov = (float *) malloc ((nchems + 1) * sizeof (float));  //next time to update any load function for chemical type [i]
    ...
    for (i = 1; i <= nchems; i++)
    {
        ...
        ncwovt[i] = (float *) malloc ((ncwov[i] + 1) * sizeof (float)); //next (upper) interval time for chemical load time series
        pcwovt[i] = (float *) malloc ((ncwov[i] + 1) * sizeof (float)); //prior (lower) interval time for chemical load time series
        bcwov[i] = (float *) malloc ((ncwov[i] + 1) * sizeof (float));  //intercept for chemical load interpolation
        mcwov[i] = (float *) malloc ((ncwov[i] + 1) * sizeof (float));  //slope for chemical load interpolation
        cwovpoint[i] = (int *) malloc ((ncwov[i] + 1) * sizeof (int));  //pointer (to position in time series) for chemical loads
        cwovinterp[i] = (float *) malloc ((ncwov[i] + 1) * sizeof (float));     //external chemical load interpolated in time for source[i]
    ...
    for (i = 1; i <= nchems; i++)
    {
        ...
        if (chemexpfile_fp == NULL)
        {
            ...
            printf
                ("Chemical Type = %d  Can't create Chemical Export File: %s \n",
                 i, chemexpfile[i]);
    ...
    //
    //Allocate memory for chemical grid file names (total, dissolved, bound, and particulate)
    totchemconcwatergrid =
        (char **) malloc ((ncgroups + 1) * sizeof (char *));
    dischemconcwatergrid =
        (char **) malloc ((ncgroups + 1) * sizeof (char *));
    bndchemconcwatergrid =
        (char **) malloc ((ncgroups + 1) * sizeof (char *));
    prtchemconcwatergrid =
        (char **) malloc ((ncgroups + 1) * sizeof (char *));
    ...
    dischemfracwatergrid =
        (char **) malloc ((ncgroups + 1) * sizeof (char *));
    bndchemfracwatergrid =
        (char **) malloc ((ncgroups + 1) * sizeof (char *));
    mblchemfracwatergrid =
        (char **) malloc ((ncgroups + 1) * sizeof (char *));
    prtchemfracwatergrid =
        (char **) malloc ((ncgroups + 1) * sizeof (char *));
    ...
    for (i = 1; i <= ncgroups; i++)
    {
        ...
        totchemconcwatergrid[i] =
            (char *) malloc (MAXNAMESIZE * sizeof (char));
        dischemconcwatergrid[i] =
            (char *) malloc (MAXNAMESIZE * sizeof (char));
        bndchemconcwatergrid[i] =
            (char *) malloc (MAXNAMESIZE * sizeof (char));
        prtchemconcwatergrid[i] =
            (char *) malloc (MAXNAMESIZE * sizeof (char));
        totchemconcsurfgrid[i] =
            (char *) malloc (MAXNAMESIZE * sizeof (char));
        dischemconcsurfgrid[i] =
            (char *) malloc (MAXNAMESIZE * sizeof (char));
        bndchemconcsurfgrid[i] =
            (char *) malloc (MAXNAMESIZE * sizeof (char));
        prtchemconcsurfgrid[i] =
            (char *) malloc (MAXNAMESIZE * sizeof (char));
        dischemfracwatergrid[i] =
            (char *) malloc (MAXNAMESIZE * sizeof (char));
        bndchemfracwatergrid[i] =
            (char *) malloc (MAXNAMESIZE * sizeof (char));
        mblchemfracwatergrid[i] =
            (char *) malloc (MAXNAMESIZE * sizeof (char));
        prtchemfracwatergrid[i] =
            (char *) malloc (MAXNAMESIZE * sizeof (char));
        dischemfracsurfgrid[i] =
            (char *) malloc (MAXNAMESIZE * sizeof (char));
        bndchemfracsurfgrid[i] =
            (char *) malloc (MAXNAMESIZE * sizeof (char));
        mblchemfracsurfgrid[i] =
            (char *) malloc (MAXNAMESIZE * sizeof (char));
        prtchemfracsurfgrid[i] =
            (char *) malloc (MAXNAMESIZE * sizeof (char));
        ...
        totchemconcwatergrid[i] =
            strcat (totchemconcwatergrid[i], cgroupname[i]);
        dischemconcwatergrid[i] =
            strcat (dischemconcwatergrid[i], cgroupname[i]);
        bndchemconcwatergrid[i] =
            strcat (bndchemconcwatergrid[i], cgroupname[i]);
        prtchemconcwatergrid[i] =
            strcat (prtchemconcwatergrid[i], cgroupname[i]);
        totchemconcsurfgrid[i] =
            strcat (totchemconcsurfgrid[i], cgroupname[i]);
        dischemconcsurfgrid[i] =
            strcat (dischemconcsurfgrid[i], cgroupname[i]);
        bndchemconcsurfgrid[i] =
            strcat (bndchemconcsurfgrid[i], cgroupname[i]);
        prtchemconcsurfgrid[i] =
            strcat (prtchemconcsurfgrid[i], cgroupname[i]);
        dischemfracwatergrid[i] =
            strcat (dischemfracwatergrid[i], cgroupname[i]);
        bndchemfracwatergrid[i] =
            strcat (bndchemfracwatergrid[i], cgroupname[i]);
        mblchemfracwatergrid[i] =
            strcat (mblchemfracwatergrid[i], cgroupname[i]);
        prtchemfracwatergrid[i] =
            strcat (prtchemfracwatergrid[i], cgroupname[i]);
        dischemfracsurfgrid[i] =
            strcat (dischemfracsurfgrid[i], cgroupname[i]);
        bndchemfracsurfgrid[i] =
            strcat (bndchemfracsurfgrid[i], cgroupname[i]);
        mblchemfracsurfgrid[i] =
            strcat (mblchemfracsurfgrid[i], cgroupname[i]);
        prtchemfracsurfgrid[i] =
            strcat (prtchemfracsurfgrid[i], cgroupname[i]);
    ...
    {
        //Allocate initial memory for chemical properties
        fdissolvedch = (float ****) malloc ((nchems + 1) * sizeof (float ***)); //fraction dissolved of a chemical in channels [ichem][row][col][layer]
        fboundch = (float ****) malloc ((nchems + 1) * sizeof (float ***));     //fraction bound of a chemical in channels [ichem][row][col][layer]
        fparticulatech = (float *****) malloc ((nchems + 1) * sizeof (float ****));     //fraction particulate of a chemical in channels [ichem][isolid][row][col][layer]

        //Loop over chemicals
        for (ichem = 1; ichem <= nchems; ichem++)
        {
            //Allocate additional memory for chemical properties
            fdissolvedch[ichem] = (float ***) malloc ((nlinks + 1) * sizeof (float **));        //fraction dissolved of a chemical in channels [ichem][row][col][layer]
            fboundch[ichem] = (float ***) malloc ((nlinks + 1) * sizeof (float **));    //fraction bound of a chemical in channels [ichem][row][col][layer]

            //Loop over number of links
            for (i = 1; i <= nlinks; i++)
            {
                //Allocate additional memory for chemical properties
                fdissolvedch[ichem][i] = (float **) malloc ((nnodes[i] + 1) * sizeof (float *));        //fraction dissolved of a chemical in channels [ichem][row][col][layer]
                fboundch[ichem][i] = (float **) malloc ((nnodes[i] + 1) * sizeof (float *));    //fraction bound of a chemical in channels [ichem][row][col][layer]

                //Loop over number of nodes
                for (j = 1; j <= nnodes[i]; j++)
                {
                    //Allocate additional memory for chemical properties
                    fdissolvedch[ichem][i][j] = (float *) malloc ((maxstackch + 1) * sizeof (float));   //fraction dissolved of a chemical in channels [ichem][row][col][layer]
                    fboundch[ichem][i][j] = (float *) malloc ((maxstackch + 1) * sizeof (float));       //fraction bound of a chemical in channels [ichem][row][col][layer]
            ...
            //Allocate additional memory for chemical properties
            fparticulatech[ichem] = (float ****) malloc ((nsolids + 1) * sizeof (float ***));   //fraction particulate of a chemical in channels [ichem][row][col][layer]

            //Loop over number of solids
            for (isolid = 1; isolid <= nsolids; isolid++)
            {
                //Allocate additional memory for chemical properties
                fparticulatech[ichem][isolid] = (float ***) malloc ((nlinks + 1) * sizeof (float **));  //fraction particulate of a chemical in channels [ichem][row][col][layer]

                //Loop over number of links
                for (i = 1; i <= nlinks; i++)
                {
                    //Allocate additional memory for chemical properties
                    fparticulatech[ichem][isolid][i] = (float **) malloc ((nnodes[i] + 1) * sizeof (float *));  //fraction dissolved of a chemical in channels [ichem][row][col][layer]

                    //Loop over number of nodes
                    for (j = 1; j <= nnodes[i]; j++)
                    {
                        //Allocate additional memory for chemical properties
                        fparticulatech[ichem][isolid][i][j] = (float *) malloc ((maxstackch + 1) * sizeof (float));     //fraction particulate of a chemical in channels [ichem][row][col][layer]
    ...
    for (i = 1; i <= nrows; i++)
    {
        ...
        for (j = 1; j <= ncols; j++)
        {
            ...
            if (imask[i][j] > 0)
            {
                ...
                {
                    //compute concentration of all solids types in this cell...
                    //
                    //initialize sum of total bed solids (tbs) concentrations (g/m3)
                    tbs = 0.0;

                    //loop over solids
                    for (isolid = 1; isolid <= nsolids; isolid++)
                    {
                        //sum of total bed solids (tbs) concentrations (g/m3)
                        tbs = tbs + csedov[isolid][i][j][k];
                    ...
                    for (ichem = 1; ichem <= nchems; ichem++)
                    {
                        ...
                        cchemov[ichem][i][j][k] =
                            (float) (cchemov[ichem][i][j][k] * tbs * 1.0e-6);
            ...
            for (j = 1; j <= nnodes[i]; j++)
            {
                ...
                for (k = 1; k <= nstackch[i][j]; k++)
                {
                    ...
                    for (ichem = 1; ichem <= nchems; ichem++)
                    {
                        //convert from ug/g (mg/kg) to g/m3
                        cchemch[ichem][i][j][k] =
                            (float) (cchemch[ichem][i][j][k] * tbs * 1.0e-6);
