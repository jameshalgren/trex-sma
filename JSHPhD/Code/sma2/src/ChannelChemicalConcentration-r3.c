...
void ChannelChemicalConcentration(void)
{
    ...
    float hchan,                //new depth of water in channel (at time t + dt) (m)
    ...
    double influx,              //sum of all influx terms for derivative (g/s)
      ...
      sedmass,                  //surface sediment layer chemical mass of node (at time t) (g)
      newsedmass,               //new surface sediment layer chemical mass of node (at time t + dt) (g)
    ...
    for (i = 1; i <= nlinks; i++)
    {
        ...
        for (j = 1; j <= nnodes[i]; j++)
        {
            ...
            for (ichem = 1; ichem <= nchems; ichem++)
            {
                ...
                //
                //  derivative = advectionin[ichem][i][j][source=0-8,10]
                //             - advectionout[ichem][i][j][source=0-8,10]
                //             + dispersionin[ichem][i][j][source=0-8,10]
                //             - dispersionout[ichem][i][j][source=0-8,10]
                ...
                {
                    //compute the cumulative inflowing chemical mass (kg)
                    advchemchinmass[ichem][i][j][k] =
                        advchemchinmass[ichem][i][j][k] +
                        (float) (advchemchinflux[ichem][i][j][k] * dt[idt] /
                                 1000.0);

                    //compute the cumulative outflowing chemical mass (kg)
                    advchemchoutmass[ichem][i][j][k] =
                        advchemchoutmass[ichem][i][j][k] +
                        (float) (advchemchoutflux[ichem][i][j][k] * dt[idt] /
                                 1000.0);

                    //compute the cumulative in-dispersing chemical mass (kg)
                    dspchemchinmass[ichem][i][j][k] =
                        dspchemchinmass[ichem][i][j][k] +
                        (float) (dspchemchinflux[ichem][i][j][k] * dt[idt] /
                                 1000.0);

                    //compute the cumulative out-dispersing chemical mass (kg)
                    dspchemchoutmass[ichem][i][j][k] =
                        dspchemchoutmass[ichem][i][j][k] +
                        (float) (dspchemchoutflux[ichem][i][j][k] * dt[idt] /
                                 1000.0);
                ...
                if (qchoutlet[i] > 0 && j == nnodes[i])
                {
                    ...
                    advchemchinmass[ichem][i][j][10] =
                        advchemchinmass[ichem][i][j][10] +
                        (float) (advchemchinflux[ichem][i][j][10] * dt[idt] /
                                 1000.0);

                    //compute the cumulative outflowing chemical mass (kg)
                    advchemchoutmass[ichem][i][j][10] =
                        advchemchoutmass[ichem][i][j][10] +
                        (float) (advchemchoutflux[ichem][i][j][10] * dt[idt] /
                                 1000.0);

                    //compute the cumulative in-dispersing chemical mass (kg)
                    dspchemchinmass[ichem][i][j][10] =
                        dspchemchinmass[ichem][i][j][10] +
                        (float) (dspchemchinflux[ichem][i][j][10] * dt[idt] /
                                 1000.0);

                    //compute the cumulative out-dispersing chemical mass (kg)
                    dspchemchoutmass[ichem][i][j][10] =
                        dspchemchoutmass[ichem][i][j][10] +
                        (float) (dspchemchoutflux[ichem][i][j][10] * dt[idt] /
                                 1000.0);

                    //Compute the cumulative chemical mass exiting the domain at this outlet (kg)
                    totaladvchemoutch[ichem][qchoutlet[i]] =
                        totaladvchemoutch[ichem][qchoutlet[i]] +
                        (float) (advchemchoutflux[ichem][i][j][10] * dt[idt] /
                                 1000.0);

                    //Compute the cumulative chemical mass entering the domain at this outlet (kg)
                    totaladvcheminch[ichem][qchoutlet[i]] =
                        totaladvcheminch[ichem][qchoutlet[i]] +
                        (float) (advchemchinflux[ichem][i][j][10] * dt[idt] /
                                 1000.0);

                    //Compute the cumulative chemical mass exiting the domain at this outlet (kg)
                    totaldspchemoutch[ichem][qchoutlet[i]] =
                        totaldspchemoutch[ichem][qchoutlet[i]] +
                        (float) (dspchemchoutflux[ichem][i][j][10] * dt[idt] /
                                 1000.0);

                    //Compute the cumulative chemical mass entering the domain at this outlet (kg)
                    totaldspcheminch[ichem][qchoutlet[i]] =
                        totaldspcheminch[ichem][qchoutlet[i]] +
                        (float) (dspchemchinflux[ichem][i][j][10] * dt[idt] /
                                 1000.0);
                    ...
                    if (chemflow > chemflowpeakch[ichem][qchoutlet[i]])
                    {
                        ...
                        chemtimepeakch[ichem][qchoutlet[i]] =
                            (float) (simtime);
                ...
                if (newwatervol > 0.0)
                {
                    ...
                    cchemchnew[ichem][i][j][0] =
                        (float) (newmassch / newwatervol);
                }
            ...
            //Compute derivative for surface sediment layer...
            //
            //assign surface sediment layer volume (m3)
            sedvol = vlayerch[i][j][ilayer];

            //assign new surface sediment layer volume (m3)
            ...
            for (ichem = 1; ichem <= nchems; ichem++)
            {
                ...
                if (newsedvol >= TOLERANCE)
                {
                    ...
                    depchemchinmass[ichem][i][j][ilayer] =
                        depchemchinmass[ichem][i][j][ilayer] +
                        (float) (depchemchinflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute cumulative chemical mass leaving sediment bed by erosion (kg)
                    erschemchoutmass[ichem][i][j][ilayer] =
                        erschemchoutmass[ichem][i][j][ilayer] +
                        (float) (erschemchoutflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute cumulative chemical mass leaving sediment bed by porewater release (kg)
                    pwrchemchoutmass[ichem][i][j][ilayer] =
                        pwrchemchoutmass[ichem][i][j][ilayer] +
                        (float) (pwrchemchoutflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute the cumulative chemical mass entering sediment bed by infiltration (kg)
                    infchemchinmass[ichem][i][j][ilayer] =
                        infchemchinmass[ichem][i][j][ilayer] +
                        (float) (infchemchinflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute cumulative chemical mass leaving sediment bed by biodegradation (kg)
                    biochemchoutmass[ichem][i][j][ilayer] =
                        biochemchoutmass[ichem][i][j][ilayer] +
                        (float) (biochemchoutflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute the cumulative chemical mass entering sediment bed by biodegradation (kg)
                    biochemchinmass[ichem][i][j][ilayer] =
                        biochemchinmass[ichem][i][j][ilayer] +
                        (float) (biochemchinflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute cumulative chemical mass leaving sediment bed by hydrolysis (kg)
                    hydchemchoutmass[ichem][i][j][ilayer] =
                        hydchemchoutmass[ichem][i][j][ilayer] +
                        (float) (hydchemchoutflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute the cumulative chemical mass entering sediment bed by hydrolysis (kg)
                    hydchemchinmass[ichem][i][j][ilayer] =
                        hydchemchinmass[ichem][i][j][ilayer] +
                        (float) (hydchemchinflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute cumulative chemical mass leaving sediment bed by oxidation (kg)
                    oxichemchoutmass[ichem][i][j][ilayer] =
                        oxichemchoutmass[ichem][i][j][ilayer] +
                        (float) (oxichemchoutflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute the cumulative chemical mass entering sediment bed by oxidation (kg)
                    oxichemchinmass[ichem][i][j][ilayer] =
                        oxichemchinmass[ichem][i][j][ilayer] +
                        (float) (oxichemchinflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute cumulative chemical mass leaving bed by photolysis (kg)
                    phtchemchoutmass[ichem][i][j][ilayer] =
                        phtchemchoutmass[ichem][i][j][ilayer] +
                        (float) (phtchemchoutflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute the cumulative chemical mass entering sediment bed by photolysis (kg)
                    phtchemchinmass[ichem][i][j][ilayer] =
                        phtchemchinmass[ichem][i][j][ilayer] +
                        (float) (phtchemchinflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute cumulative chemical mass leaving sediment bed by radioactive decay (kg)
                    radchemchoutmass[ichem][i][j][ilayer] =
                        radchemchoutmass[ichem][i][j][ilayer] +
                        (float) (radchemchoutflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute the cumulative chemical mass entering sediment bed by radioactive decay (kg)
                    radchemchinmass[ichem][i][j][ilayer] =
                        radchemchinmass[ichem][i][j][ilayer] +
                        (float) (radchemchinflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute cumulative chemical mass leaving sediment bed by volatilization (kg)
                    vltchemchoutmass[ichem][i][j][ilayer] =
                        vltchemchoutmass[ichem][i][j][ilayer] +
                        (float) (vltchemchoutflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute the cumulative chemical mass entering sediment bed by volatilization (kg)
                    vltchemchinmass[ichem][i][j][ilayer] =
                        vltchemchinmass[ichem][i][j][ilayer] +
                        (float) (vltchemchinflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute cumulative chemical mass leaving sediment bed by user-defined reaction (kg)
                    udrchemchoutmass[ichem][i][j][ilayer] =
                        udrchemchoutmass[ichem][i][j][ilayer] +
                        (float) (udrchemchoutflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);

                    //compute the cumulative chemical mass entering sediment bed by user-defined reaction (kg)
                    udrchemchinmass[ichem][i][j][ilayer] =
                        udrchemchinmass[ichem][i][j][ilayer] +
                        (float) (udrchemchinflux[ichem][i][j][ilayer] *
                                 dt[idt] / 1000.0);
                ...
                //                   sediment layers, derivative terms
