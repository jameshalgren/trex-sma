...
void OverlandChemicalAdvection(void)
{
    ...
    float cin[11],              //chemical concentration of inflowing cell/node (source 0-10)
        ...
        for (j = 1; j <= ncols; j++)
        {
            ...
            if (imask[i][j] != nodatavalue)
            {
                ...
                for (ichem = 1; ichem <= nchems; ichem++)
                {
                    ...
                    for (k = 1; k <= ncwov[ichem]; k++)
                    {
                        //if the current cell has a point source (row/col match)
                        if (cwovrow[ichem][k] == i && cwovcol[ichem][k] == j)
                        {
                            ...
                            if (load >= 0.0)
                            {
                                ...
                                advchemovinflux[ichem][i][j][0] = load;
                            ...
                            else        //else the load is negative (a sink)
                            {
                                ...
                                advchemovoutflux[ichem][i][j][0] = load;

                            }   //end if load >= 0.0

                        }       //end if cwovrow = i and cwovcol = j

                    }           //end loop over point sources

                    //Compute Advective Flows and Fluxes...
                    //
                    ...
                    for (k = 1; k <= 7; k = k + 2)
                    {
                        ...
                        advchemovoutflux[ichem][i][j][k] =
                            advoutflowov[i][j][k] * cchemov[ichem][i][j][0];
    ...
    for (i = 1; i <= noutlets; i++)
    {
        ...
        for (ichem = 1; ichem <= nchems; ichem++)
        {
            ...
            potential =
                advchemovoutflux[ichem][iout[i]][jout[i]][10] * dt[idt];
