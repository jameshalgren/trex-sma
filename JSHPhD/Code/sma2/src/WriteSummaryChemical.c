...
void WriteSummaryChemical(void)
{
    //local variable declarations
    int ichem;                  //loop index for chemical type

    double *initialmass,        //initial mass in domain (m3) [chem]
    ...
    for (ichem = 1; ichem <= nchems; ichem++)
    {
        ...
        if (chnopt > 0)
        {
            ...
            finalmass[ichem] = finalmass[ichem] + finalchemmassch[ichem][0];
        ...
        totalmassin[ichem] = totalmassin[ichem] + totalerscheminov[ichem];

        //Add cumulative mass entering the overland water column by porewater release (kg)
        totalmassin[ichem] = totalmassin[ichem] + totalpwrcheminov[ichem];
        ...
        totalmassout[ichem] = totalmassout[ichem] + totaldepchemoutov[ichem];

        //Add cumulative mass leaving the overland water column by infiltration (kg)
        totalmassout[ichem] = totalmassout[ichem] + totalinfchemoutov[ichem];
        ...
        if (chnopt > 0)
        {
            ...
            totalmassin[ichem] = totalmassin[ichem] + totalerscheminch[ichem];

            //Add cumulative mass entering the channel water column by porewater release (kg)
            totalmassin[ichem] = totalmassin[ichem] + totalpwrcheminch[ichem];
    ...
    for (ichem = 1; ichem <= nchems; ichem++)
    {
        ...
        sourcemass[ichem] = initialmass[ichem] + totalmassin[ichem];

        //Compute sum of water column chemicals sink masses (kg)
        sinkmass[ichem] = finalmass[ichem] + totalmassout[ichem];
        ...
        fprintf (statsfile_fp, "  Overland Advection (kg)...........= ");
        fprintf (statsfile_fp, "%15.2e\n", totaladvcheminov[ichem][0]);

        //if channels are simulated
        if (chnopt > 0)
        {
            fprintf (statsfile_fp, "  Channel Advection (kg)............= ");
        ...
        fprintf (statsfile_fp, "  Overland Dispersion (kg)..........= ");
        fprintf (statsfile_fp, "%15.2e\n", totaldspcheminov[ichem][0]);

        //if channels are simulated
        if (chnopt > 0)
        {
            fprintf (statsfile_fp, "  Channel Dispersion (kg)...........= ");
            fprintf (statsfile_fp, "%15.2e\n\n", totaldspcheminch[ichem][0]);
        ...
        fprintf (statsfile_fp, "  Overland Erosion (kg).............= ");
        fprintf (statsfile_fp, "%15.2e\n", totalerscheminov[ichem]);

        //if channels are simulated
        if (chnopt > 0)
        {
            fprintf (statsfile_fp, "  Channel Erosion (kg)..............= ");
        ...
        fprintf (statsfile_fp, "  Overland Porewater Release (kg)...= ");
        fprintf (statsfile_fp, "%15.2e\n", totalpwrcheminov[ichem]);

        //if channels are simulated
        if (chnopt > 0)
        {
            fprintf (statsfile_fp, "  Channel Porewater Release (kg)....= ");
        ...
        fprintf (statsfile_fp, "  Overland Transformations (kg).....= ");
        fprintf (statsfile_fp, "%15.2e\n", totaltnscheminov[ichem]);

        //if channels are simulated
        if (chnopt > 0)
        {
            fprintf (statsfile_fp, "  Channel Transformations (kg)......= ");
        ...
        fprintf (statsfile_fp, "  Overland Sources (kg).............= ");
        fprintf (statsfile_fp, "%15.2e\n", totalcwov[ichem]);

        //if channels are simulated
        if (chnopt > 0)
        {
            fprintf (statsfile_fp, "  Channel Sources (kg)..............= ");
            fprintf (statsfile_fp, "%15.2e\n\n", totalcwch[ichem]);
        ...
        fprintf (statsfile_fp, "  Overland Deposition (kg)..........= ");
        fprintf (statsfile_fp, "%15.2e\n", totaldepchemoutov[ichem]);

        //if channels are simulated
        if (chnopt > 0)
        {
            fprintf (statsfile_fp, "  Channel Deposition (kg)...........= ");
            fprintf (statsfile_fp, "%15.2e\n\n", totaldepchemoutch[ichem]);
        ...
        {
            fprintf (statsfile_fp, "%15.2e\n",
                     totalinfchemoutov[ichem] + totalinfchemoutch[ichem]);
        }
        else                    //else channels are not simulated
        {
            fprintf (statsfile_fp, "%15.2e\n", totalinfchemoutov[ichem]);

        }                       //end if channels are simulated

        fprintf (statsfile_fp, "  Overland Infiltration (kg)........= ");
        fprintf (statsfile_fp, "%15.2e\n", totalinfchemoutov[ichem]);

        //if channels are simulated
        if (chnopt > 0)
        {
            fprintf (statsfile_fp, "  Channel Infiltration (kg).........= ");
            fprintf (statsfile_fp, "%15.2e\n\n", totalinfchemoutch[ichem]);
        ...
        fprintf (statsfile_fp,
                 "Min. Overland Surface Soil Total Chemical Concentration (g/m3).....= ");
        fprintf (statsfile_fp, "%15.2e\n", mincchemov1[ichem]);

        fprintf (statsfile_fp,
                 "Max. Overland Surface Soil Total Chemical Concentration (g/m3).....= ");
        fprintf (statsfile_fp, "%15.2e\n\n", maxcchemov1[ichem]);
        ...
        if (chnopt > 0)
        {
            ...
            fprintf (statsfile_fp,
                     "Min. Channel Surface Sediment Total Chemical Concentration (g/m3)..= ");
            fprintf (statsfile_fp, "%15.2e\n", mincchemch1[ichem]);

            fprintf (statsfile_fp,
                     "Max. Channel Surface Sediment Total Chemical Concentration (g/m3)..= ");
            fprintf (statsfile_fp, "%15.2e\n\n", maxcchemch1[ichem]);
        }
        else                    //else channels are not simulated
        {
