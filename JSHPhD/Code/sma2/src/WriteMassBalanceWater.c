...
void WriteMassBalanceWater(void)
{
    ...
    fprintf (msbfile_fp, "\nrow\tcol\timask\tlink\tnode\tinitial vol (m3)");
    fprintf (msbfile_fp, "\tgrossrain (m3)\tinterception (m3)");
    fprintf (msbfile_fp, "\tnetrain (m3)\tinfiltration (m3)");
    ...
    for (i = 1; i <= nrows; i++)
    {
        ...
        for (j = 1; j <= ncols; j++)
        {
            ...
            if (imask[i][j] != nodatavalue)
            {
                ...
                fprintf (msbfile_fp, "\t%f\t%f\t%f",
                         grossrainvol[i][j] * (w * w - achsurf) / (w * w),
                         interceptionvol[i][j],
                         netrainvol[i][j] * (w * w - achsurf) / (w * w));

                //Developer's Note: snowmelt would be added here...
                //
//mlv                           //if snowmelt is simulated
//mlv                           if(infopt > 0)
//mlv                           {
//mlv                                   //write snowmelt summary
//mlv                                   fprintf(msbfile_fp,"\t%12.3f", infiltrationvol[i][j]);
//mlv                           }
//mlv                           else    //else infiltration is not simulated
//mlv                           {
//mlv                                   //write blank (zero) snowmelt summary
//mlv                                   fprintf(msbfile_fp,"\t%12.3f", 0.0);
//mlv
//mlv                           }       //end if infopt > 0
...
//mlv:  no. just report one valid upstream link even if there are more...
                ...
                //Developer's Note: transmission losses (infiltration
                //                  through the channel bed) would be
                //                  added here...
