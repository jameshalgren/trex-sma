...
void ComputeFinalStateChemical(void)
{
            ...
            if (imask[i][j] != nodatavalue)
            {
                ...
                for (ichem = 1; ichem <= nchems; ichem++)
                {
                    ...
                    finalchemov[ichem][i][j][0] =
                        (float) (cchemov[ichem][i][j][0] * hov[i][j] *
                                 (w * w - achsurf) / 1000.0);
                    ...
                    totalinfchemoutov[ichem] = totalinfchemoutov[ichem]
            ...
            for (j = 1; j <= nnodes[i]; j++)
            {
                ...
                for (ichem = 1; ichem <= nchems; ichem++)
                {
                    ...
                    finalchemch[ichem][i][j][0] =
                        (float) (cchemch[ichem][i][j][0] * achcross * lch /
                                 1000.0);
                    ...
                    //compute the cumulative chemical mass leaving water column by infiltration over domain (kg)
                    totalinfchemoutch[ichem] = totalinfchemoutch[ichem]
