...
void ReadDataGroupD(void)
{
    //local variable declarations/definitions
    int ichem,                  //loop counter for chemical number
    ...
    float lkp,                  //temporary variable to store log kp partitioning values
    ...
    cgroupnumber = (int *) malloc ((ncgroups + 1) * sizeof (int));      //chemical reporting group number
...
*/
    ...
    if (chnopt > 0)
    {
        ...
        for (ichem = 1; ichem <= nchems; ichem++)
        {
            ...
            cchemch[ichem] =
                (float ***) malloc ((nlinks + 1) * sizeof (float **));
            ...
            for (i = 1; i <= nlinks; i++)
            {
                ...
                for (j = 1; j <= nnodes[i]; j++)
                {
                    ...
                    cchemch[ichem][i][j] =
                        (float *) malloc ((maxstackch + 1) * sizeof (float));
    ...
    //Write label for overland chemical loads/forcing functions
    fprintf (echofile_fp,
             "\n\n\n  Overland Chemical Loads/Forcing Fucntions  \n");
    fprintf (echofile_fp,
             "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n");

    //Allocate memory for overland chemical loads/forcing functions (w)
    ncwov = (int *) malloc ((nchems + 1) * sizeof (int));       //allocate memory
    cwovrow = (int **) malloc ((nchems + 1) * sizeof (int *));  //allocate memory
    cwovcol = (int **) malloc ((nchems + 1) * sizeof (int *));  //allocate memory
    ncwovpairs = (int **) malloc ((nchems + 1) * sizeof (int *));       //allocate memory
    cwovopt = (int **) malloc ((nchems + 1) * sizeof (int *));  //allocate memory
    cwov = (float ***) malloc ((nchems + 1) * sizeof (float **));       //allocate memory
    cwovtime = (float ***) malloc ((nchems + 1) * sizeof (float **));   //allocate memory
    ...
    for (i = 1; i <= nchems; i++)
    {
        ...
        fscanf (inputfile_fp, "%s %d",  //read
                ...
                &ncwov[i]);     //number of overland chemical loads for current chemical type

        //Echo number of loads and description to file
        fprintf (echofile_fp, "\n\n %s %d  %s %d  %s %s\n",
                 "Chemical Type: ", i, "Number of Loads: ", ncwov[i],
        ...
        if (ncwov[i] > 0)
        {
            ...
            //Allocate additional memory for overland chemical loads/forcing functions
            cwovrow[i] = (int *) malloc ((ncwov[i] + 1) * sizeof (int));        //allocate memory
            cwovcol[i] = (int *) malloc ((ncwov[i] + 1) * sizeof (int));        //allocate memory
            ncwovpairs[i] = (int *) malloc ((ncwov[i] + 1) * sizeof (int));     //allocate memory
            cwovopt[i] = (int *) malloc ((ncwov[i] + 1) * sizeof (int));        //allocate memory
            cwov[i] = (float **) malloc ((ncwov[i] + 1) * sizeof (float *));    //allocate memory
            cwovtime[i] = (float **) malloc ((ncwov[i] + 1) * sizeof (float *));        //allocate memory

            //Loop over number of overland loads for current chemical type
            for (j = 1; j <= ncwov[i]; j++)
            {
                //Record 20 (Part 1)
                fscanf (inputfile_fp, "%d %d %d %d",    //read
                        &cwovrow[i][j], //overland soilds load row
                        &cwovcol[i][j], //overland soilds load column
                        &ncwovpairs[i][j],      //number of pairs (time breaks) in function
                        &cwovopt[i][j]);        //overland chemical load option
                ...
                fprintf (echofile_fp,
                         ...
                         cwovrow[i][j], "  Column Number: ", cwovcol[i][j],
                         "  Number of Time Breaks: ", ncwovpairs[i][j],
                         "  Load Option: ", cwovopt[i][j], "  Description: ",
                         loadname);

                //Allocate remaining memory for chemical loads/forcing functions
                cwov[i][j] = (float *) malloc ((ncwovpairs[i][j] + 1) * sizeof (float));        //allocate memory
                cwovtime[i][j] = (float *) malloc ((ncwovpairs[i][j] + 1) * sizeof (float));    //allocate memory
                ...
                if (cwovopt[i][j] == 1)
                {
                    fprintf (echofile_fp, "  Concentration (g/m3)     ");       //load in g/m3 (concentration) when cwovopt = 1
                ...
                {
                    fprintf (echofile_fp, "  Load (kg/day)     ");      //load in kg/day when cwovopt = 0
                    fprintf (echofile_fp, "  Time (days)  \n");
                    fprintf (echofile_fp, "~~~~~~~~~~~~~~~~~   ");
                    fprintf (echofile_fp, "~~~~~~~~~~~~~~~\n");
                ...
                for (k = 1; k <= ncwovpairs[i][j]; k++)
                {
                    //Record 21
                    fscanf (inputfile_fp, "%f %f",      //read
                            &cwov[i][j][k],     //chemical load value
                            &cwovtime[i][j][k]);        //chemical load time break

                    //Apply conversion and scale factors
                    cwov[i][j][k] =
                        cwov[i][j][k] * convunits * convtime * scale;

                    //Echo load value and time break to file
                    fprintf (echofile_fp, "%16.3f %19.3f\n",
                             cwov[i][j][k], cwovtime[i][j][k]);
    ...
    if (chnopt > 0)
    {
        ...
        for (i = 1; i <= nchems; i++)
        {
            ...
            //Echo number of loads and description to file
            fprintf (echofile_fp, "\n\n %s %d  %s %d  %s %s\n",
                     "Solids Type:", i, "Number of Loads:", ncwch[i],
                     "Description:", loadname);
            ...
            if (ncwch[i] > 0)
            {
                ...
                for (j = 1; j <= ncwch[i]; j++)
                {
                    ...
                    fprintf (echofile_fp,
                             ...
                             cwchlink[i][j], "  Node Number: ",
                             cwchnode[i][j], "  Number of Time Breaks: ",
                             ncwchpairs[i][j], "  Load Option: ",
                             cwchopt[i][j], "  Description: ", loadname);
                    ...
                    for (k = 1; k <= ncwchpairs[i][j]; k++)
                    {
                        ...
                        //Apply conversion and scale factors
                        cwch[i][j][k] =
                            cwch[i][j][k] * convunits * convtime * scale;
    ...
    for (i = 1; i <= noutlets; i++)
    {
        ...
        if (dbcopt[i] > 0)
        {
            ...
            {
                //Record 28 (Part 1)
                fscanf (inputfile_fp, "%d", &ncbcpairs[i][j]);  //Number of pairs for this chemical boundary conditions
                ...
                fprintf (echofile_fp,
                         ...
                         "  Column Number: ", jout[i], "  Chemical Type: ", j,
                ...
                for (k = 1; k <= ncbcpairs[i][j]; k++)
                {
                    ...
                    //Apply conversion and scale factors
                    cbc[i][j][k] =
                        cbc[i][j][k] * convunits * convtime * scale;
    ...
    for (i = 1; i <= nchemreports; i++)
    {
        ...
        fprintf (echofile_fp, "%11d   %12d   %15d   %23.3f   %23d %s\n",
                 i, chemreprow[i], chemrepcol[i], chemarea[i],
                 chemunitsopt[i], varname);
