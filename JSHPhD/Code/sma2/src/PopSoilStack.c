...
void PopSoilStack(void)
{
    ...
    double residualmass,        //residual solids mass in eroded surface layer (g)
    ...
    for (i = 1; i <= nrows; i++)
    {
        ...
        for (j = 1; j <= ncols; j++)
        {
            ...
            if (imask[i][j] != nodatavalue)
            {
                ...
                if (vlayerov[i][j][ilayer] <= minvlayerov[i][j][ilayer]
                    && ilayer > 1)
                {
                    //Pop the stack...
                    //
                    ...
                    if (imask[i][j] > 1)
                    {
                        ...
                        if (hbank[chanlink][channode] <= 0.0)
                        {
                            ...
                            SimulationError (7, i, j, nstackov[i][j] + 1);
