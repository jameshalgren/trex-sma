...
void ChannelSolidsConcentration(void)
{
        ...
        for (j = 1; j <= nnodes[i]; j++)
        {
            ...
            for (isolid = 1; isolid <= nsolids; isolid++)
            {
                ...
                depsedchoutmass[isolid][i][j][0] =
                    depsedchoutmass[isolid][i][j][0] +
                    (float) (depsedchoutflux[isolid][i][j][0] * dt[idt] /
                             1000.0);

                //compute the cumulative sediment mass entering the sediment bed by deposition (kg)
                depsedchinmass[isolid][i][j][ilayer] =
                    depsedchinmass[isolid][i][j][ilayer] +
                    (float) (depsedchinflux[isolid][i][j][ilayer] * dt[idt] /
                             1000.0);

                //compute cumulative solids mass leaving the bed (kg)
                erssedchoutmass[isolid][i][j][ilayer] =
                    erssedchoutmass[isolid][i][j][ilayer] +
                    (float) (erssedchoutflux[isolid][i][j][ilayer] * dt[idt] /
                             1000.0);

                //compute the cumulative solids mass entering the water column (kg)
                erssedchinmass[isolid][i][j][0] =
                    erssedchinmass[isolid][i][j][0] +
                    (float) (erssedchinflux[isolid][i][j][0] * dt[idt] /
                             1000.0);

                //Compute masses for loads (source = 0)
                //
                //compute the cumulative inflowing sediment mass (kg)
                advsedchinmass[isolid][i][j][0] =
                    advsedchinmass[isolid][i][j][0] +
                    (float) (advsedchinflux[isolid][i][j][0] * dt[idt] /
                             1000.0);

                //compute the cumulative outflowing sediment mass (kg)
                advsedchoutmass[isolid][i][j][0] =
                    advsedchoutmass[isolid][i][j][0] +
                    (float) (advsedchoutflux[isolid][i][j][0] * dt[idt] /
                             1000.0);
                ...
                totalswch[isolid] = totalswch[isolid]
                    + (float) ((advsedchinflux[isolid][i][j][0]
                                -
                                advsedchoutflux[isolid][i][j][0]) * dt[idt] /
                               1000.0);
                ...
                if (qchoutlet[i] > 0 && j == nnodes[i])
                {
                    ...
                    advsedchinmass[isolid][i][j][10] =
                        advsedchinmass[isolid][i][j][10] +
                        (float) (advsedchinflux[isolid][i][j][10] * dt[idt] /
                                 1000.0);

                    //compute the cumulative outflowing sediment mass (kg)
                    advsedchoutmass[isolid][i][j][10] =
                        advsedchoutmass[isolid][i][j][10] +
                        (float) (advsedchoutflux[isolid][i][j][10] * dt[idt] /
                                 1000.0);

                    //compute the cumulative in-dispersing sediment mass (kg)
                    dspsedchinmass[isolid][i][j][10] =
                        dspsedchinmass[isolid][i][j][10] +
                        (float) (dspsedchinflux[isolid][i][j][10] * dt[idt] /
                                 1000.0);

                    //compute the cumulative out-dispersing sediment mass (kg)
                    dspsedchoutmass[isolid][i][j][10] =
                        dspsedchoutmass[isolid][i][j][10] +
                        (float) (dspsedchoutflux[isolid][i][j][10] * dt[idt] /
                                 1000.0);

                    //Compute the cumulative solids mass exiting the domain at this outlet (kg)
                    totaladvsedoutch[isolid][qchoutlet[i]] =
                        totaladvsedoutch[isolid][qchoutlet[i]] +
                        (float) (advsedchoutflux[isolid][i][j][10] * dt[idt] /
                                 1000.0);

                    //Compute the cumulative solids mass entering the domain at this outlet (kg)
                    totaladvsedinch[isolid][qchoutlet[i]] =
                        totaladvsedinch[isolid][qchoutlet[i]] +
                        (float) (advsedchinflux[isolid][i][j][10] * dt[idt] /
                                 1000.0);

                    //Compute the cumulative solids mass exiting the domain at this outlet (kg)
                    totaldspsedoutch[isolid][qchoutlet[i]] =
                        totaldspsedoutch[isolid][qchoutlet[i]] +
                        (float) (dspsedchoutflux[isolid][i][j][10] * dt[idt] /
                                 1000.0);

                    //Compute the cumulative solids mass entering the domain at this outlet (kg)
                    totaldspsedinch[isolid][qchoutlet[i]] =
                        totaldspsedinch[isolid][qchoutlet[i]] +
                        (float) (dspsedchinflux[isolid][i][j][10] * dt[idt] /
                                 1000.0);
                    ...
                    if (sedflow > sedflowpeakch[isolid][qchoutlet[i]])
                    {
                        ...
                        sedtimepeakch[isolid][qchoutlet[i]] =
                            (float) (simtime);
            ...
            for (isolid = 1; isolid <= nsolids; isolid++)
            {
                ...
                erodedmass = erssedchoutflux[isolid][i][j][ilayer] * dt[idt];
