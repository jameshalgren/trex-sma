...
void ReadSedimentChemicalFile(void)
{
    ...
    float conc;                 //chemical concentration (ug/g = mg/kg)
    ...
    {
        printf ("Can't open Channel File: %s \n", sedimentchemicfile);  //Write message to screen
        ...
        for (j = 1; j <= nnodes[i]; j++)
        {
            ...
            if (nstack != nstackch0[i][j])
            {
                ...
                fprintf (echofile_fp,
                         "\n\n\nSediment Chemical Initial Condition File Error:\n");
                fprintf (echofile_fp,
                         "  link = %d   node = %d   nstack = %d   nstack0 = %d\n",
