...
void OverlandChemicalConcentration(void)
{
    //local variable declarations/definitions
    int ichem,                  //loop index for chemical number (type)
    ...
    float hcell,                //new depth of water in cell (at time t + dt) (m)
    ...
    float advectionin,          //gross advection influx term (in) (g/s)
    ...
    double influx,              //sum of all influx terms for derivative (g/s)
      ...
      soilmass,                 //surface soil layer chemical mass of node (at time t) (g)
      newsoilmass,              //new surface soil layer chemical mass of node (at time t + dt) (g)
      soilvol,                  //surface soil layer volume of cell (at time t) (m3)
      newsoilvol;               //new surface soil layer volume of cell (at time t + dt) (m3)

    float chemflow,             //chemical discharge (kg/s)
        ...
        for (j = 1; j <= ncols; j++)
        {
            ...
            if (imask[i][j] != nodatavalue)
            {
                ...
                for (ichem = 1; ichem <= nchems; ichem++)
                {
                    ...
                    {
                        //compute the cumulative inflowing chemical mass (kg)
                        advchemovinmass[ichem][i][j][k] =
                            advchemovinmass[ichem][i][j][k] +
                            (float) (advchemovinflux[ichem][i][j][k] *
                                     dt[idt] / 1000.0);

                        //compute the cumulative outflowing chemical mass (kg)
                        advchemovoutmass[ichem][i][j][k] =
                            advchemovoutmass[ichem][i][j][k] +
                            (float) (advchemovoutflux[ichem][i][j][k] *
                                     dt[idt] / 1000.0);

                        //compute the cumulative in-dispersing chemical mass (kg)
                        dspchemovinmass[ichem][i][j][k] =
                            dspchemovinmass[ichem][i][j][k] +
                            (float) (dspchemovinflux[ichem][i][j][k] *
                                     dt[idt] / 1000.0);

                        //compute the cumulative out-dispersing chemical mass (kg)
                        dspchemovoutmass[ichem][i][j][k] =
                            dspchemovoutmass[ichem][i][j][k] +
                            (float) (dspchemovoutflux[ichem][i][j][k] *
                                     dt[idt] / 1000.0);
                    ...
                    {
                        //compute the cumulative inflowing chemical mass (kg)
                        advchemovinmass[ichem][i][j][9] =
                            advchemovinmass[ichem][i][j][9] +
                            (float) (advchemovinflux[ichem][i][j][9] *
                                     dt[idt] / 1000.0);

                        //compute the cumulative outflowing chemical mass (kg)
                        advchemovoutmass[ichem][i][j][9] =
                            advchemovoutmass[ichem][i][j][9] +
                            (float) (advchemovoutflux[ichem][i][j][9] *
                                     dt[idt] / 1000.0);

                        //compute the cumulative in-dispersing chemical mass (kg)
                        dspchemovinmass[ichem][i][j][9] =
                            dspchemovinmass[ichem][i][j][9] +
                            (float) (dspchemovinflux[ichem][i][j][9] *
                                     dt[idt] / 1000.0);

                        //compute the cumulative out-dispersing chemical mass (kg)
                        dspchemovoutmass[ichem][i][j][9] =
                            dspchemovoutmass[ichem][i][j][9] +
                            (float) (dspchemovoutflux[ichem][i][j][9] *
                                     dt[idt] / 1000.0);
                    ...
                    //
                    //compute the cumulative inflowing chemical mass (kg)
                    advchemovinmass[ichem][i][j][10] =
                        advchemovinmass[ichem][i][j][10] +
                        (float) (advchemovinflux[ichem][i][j][10] * dt[idt] /
                                 1000.0);

                    //compute the cumulative outflowing chemical mass (kg)
                    advchemovoutmass[ichem][i][j][10] =
                        advchemovoutmass[ichem][i][j][10] +
                        (float) (advchemovoutflux[ichem][i][j][10] * dt[idt] /
                                 1000.0);

                    //compute the cumulative in-dispersing chemical mass (kg)
                    dspchemovinmass[ichem][i][j][10] =
                        dspchemovinmass[ichem][i][j][10] +
                        (float) (dspchemovinflux[ichem][i][j][10] * dt[idt] /
                                 1000.0);

                    //compute the cumulative out-dispersing chemical mass (kg)
                    dspchemovoutmass[ichem][i][j][10] =
                        dspchemovoutmass[ichem][i][j][10] +
                        (float) (dspchemovoutflux[ichem][i][j][10] * dt[idt] /
                                 1000.0);
                    ...
                    if (newwatervol > 0.0)
                    {
                        ...
                        cchemovnew[ichem][i][j][0] =
                            (float) (newmass / newwatervol);
                    }
                ...
                //
                //assign surface soil layer volume (m3)
                soilvol = vlayerov[i][j][ilayer];

                //assign new surface soil layer volume (m3)
                newsoilvol = (double) (vlayerovnew[i][j][ilayer]);
                ...
                for (ichem = 1; ichem <= nchems; ichem++)
                {
                    ...
                    if (newsoilvol >= TOLERANCE)
                    {
                        ...
                        //compute transformation influx term (g/s)
                        transformationin =
                            biochemovinflux[ichem][i][j][ilayer] +
                        ...
                        //assign the influx (g/s)
                        soilinflux =
                            deposition + porewaterin + transformationin;

                        //assign the outflux (g/s)
                        soiloutflux =
                            erosion + porewaterout + transformationout;
                        ...
                        if (potential > available)
                        {
                            //Scale the outflux potential
                            soiloutflux =
                                (available / potential) * soiloutflux;
                        ...
                        //compute cumulative chemical mass leaving soil (kg)
                        erschemovoutmass[ichem][i][j][ilayer] =
                            erschemovoutmass[ichem][i][j][ilayer] +
                            (float) (erschemovoutflux[ichem][i][j][ilayer] *
                                     dt[idt] / 1000.0);
                        ...
                        biochemovoutmass[ichem][i][j][ilayer] =
                            biochemovoutmass[ichem][i][j][ilayer] +
                            (float) (biochemovoutflux[ichem][i][j][ilayer] *
                                     dt[idt] / 1000.0);

                        //compute cumulative chemical mass gain in soil by biodegradation (kg)
                        biochemovinmass[ichem][i][j][ilayer] =
                            biochemovinmass[ichem][i][j][ilayer] +
                            (float) (biochemovinflux[ichem][i][j][ilayer] *
                                     dt[idt] / 1000.0);

                        //compute cumulative chemical mass lost in soil by hydrolysis (kg)
                        hydchemovoutmass[ichem][i][j][ilayer] =
                            hydchemovoutmass[ichem][i][j][ilayer] +
                            (float) (hydchemovoutflux[ichem][i][j][ilayer] *
                                     dt[idt] / 1000.0);

                        //compute cumulative chemical mass gain in soil by hydrolysis (kg)
                        hydchemovinmass[ichem][i][j][ilayer] =
                            hydchemovinmass[ichem][i][j][ilayer] +
                            (float) (hydchemovinflux[ichem][i][j][ilayer] *
                                     dt[idt] / 1000.0);

                        //compute cumulative chemical mass lost in soil by oxidation (kg)
                        oxichemovoutmass[ichem][i][j][ilayer] =
                            oxichemovoutmass[ichem][i][j][ilayer] +
                            (float) (oxichemovoutflux[ichem][i][j][ilayer] *
                                     dt[idt] / 1000.0);

                        //compute cumulative chemical mass gain in soil by oxidation (kg)
                        oxichemovinmass[ichem][i][j][ilayer] =
                            oxichemovinmass[ichem][i][j][ilayer] +
                            (float) (oxichemovinflux[ichem][i][j][ilayer] *
                                     dt[idt] / 1000.0);

                        //compute cumulative chemical mass lost in soil by photolysis (kg)
                        phtchemovoutmass[ichem][i][j][ilayer] =
                            phtchemovoutmass[ichem][i][j][ilayer] +
                            (float) (phtchemovoutflux[ichem][i][j][ilayer] *
                                     dt[idt] / 1000.0);

                        //compute cumulative chemical mass gain in soil by photolysis (kg)
                        phtchemovinmass[ichem][i][j][ilayer] =
                            phtchemovinmass[ichem][i][j][ilayer] +
                            (float) (phtchemovinflux[ichem][i][j][ilayer] *
                                     dt[idt] / 1000.0);

                        //compute cumulative chemical mass lost in soil by radioactive decay (kg)
                        radchemovoutmass[ichem][i][j][ilayer] =
                            radchemovoutmass[ichem][i][j][ilayer] +
                            (float) (radchemovoutflux[ichem][i][j][ilayer] *
                                     dt[idt] / 1000.0);

                        //compute cumulative chemical mass gain in soil by radioactive decay (kg)
                        radchemovinmass[ichem][i][j][ilayer] =
                            radchemovinmass[ichem][i][j][ilayer] +
                            (float) (radchemovinflux[ichem][i][j][ilayer] *
                                     dt[idt] / 1000.0);

                        //compute cumulative chemical mass lost in soil by volatilization (kg)
                        vltchemovoutmass[ichem][i][j][ilayer] =
                            vltchemovoutmass[ichem][i][j][ilayer] +
                            (float) (vltchemovoutflux[ichem][i][j][ilayer] *
                                     dt[idt] / 1000.0);

                        //compute cumulative chemical mass gain in soil by volatilization (kg)
                        vltchemovinmass[ichem][i][j][ilayer] =
                            vltchemovinmass[ichem][i][j][ilayer] +
                            (float) (vltchemovinflux[ichem][i][j][ilayer] *
                                     dt[idt] / 1000.0);

                        //compute cumulative chemical mass lost in soil by user-defined reaction (kg)
                        udrchemovoutmass[ichem][i][j][ilayer] =
                            udrchemovoutmass[ichem][i][j][ilayer] +
                            (float) (udrchemovoutflux[ichem][i][j][ilayer] *
                                     dt[idt] / 1000.0);

                        //compute cumulative chemical mass gain in soil by user-defined reaction (kg)
                        udrchemovinmass[ichem][i][j][ilayer] =
                            udrchemovinmass[ichem][i][j][ilayer] +
                            (float) (udrchemovinflux[ichem][i][j][ilayer] *
                                     dt[idt] / 1000.0);
                    ...
                    //Developer's Note:  If mass transport or transformation
                    //                   processes occur in the subsurface
                    //                   soil layers, derivative terms
                    //                   for these layers would need to be
                    //                   computed and the concentration
                    //                   updated...
    ...
    for (i = 1; i <= noutlets; i++)
    {
        ...
        for (ichem = 1; ichem <= nchems; ichem++)
        {
            ...
            totaladvchemoutov[ichem][i] = totaladvchemoutov[ichem][i]
                + (float) (advchemovoutflux[ichem][iout[i]][jout[i]][10]
                           * dt[idt] / 1000.0);
            ...
            totaladvcheminov[ichem][i] = totaladvcheminov[ichem][i]
                + (float) (advchemovinflux[ichem][iout[i]][jout[i]][10]
                           * dt[idt] / 1000.0);
