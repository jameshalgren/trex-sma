...
void InitializeSolids(void)
{
    ...
    int isoil,                  //index for soil type [i][j][ilayer]
    ...
    for (isolid = 1; isolid <= nsolids; isolid++)
    {
        ...
        dstar[isolid] = (float) (ds[isolid] *
                                 pow ((((spgravity[isolid] -
                                         1.0) * g) / pow (nu, 2.0)), 0.333));
            ...
            if (imask[i][j] > 0)
            {
                ...
                for (k = 1; k <= maxstackov; k++)
                {
                    ...
                    if (k <= nstackov0[i][j])
                    {
                        ...
                        minvlayerov[i][j][k] =
                            minvolov * (float) (vlayerov0[i][j][k]);

                        //Compute the maximum volume limit for layer push (m3)
                        maxvlayerov[i][j][k] =
                            maxvolov * (float) (vlayerov0[i][j][k]);
                        ...
                        for (isolid = 1; isolid <= nsolids; isolid++)
                        {
                            ...
                            csedov[isolid][i][j][k] =
                                (float) (gsdov[isoil][isolid] * densityp *
                                         (1.0 - porosityov[isoil]) * 1000.0);
            ...
            for (j = 1; j <= nnodes[i]; j++)
            {
                ...
                for (k = 1; k <= maxstackch; k++)
                {
                    ...
                    if (k <= nstackch0[i][j])
                    {
                        ...
                        //Compute the minimum volume limit for layer pop (m3)
                        minvlayerch[i][j][k] =
                            minvolch * (float) (vlayerch0[i][j][k]);

                        //Compute the maximum volume limit for layer push (m3)
                        maxvlayerch[i][j][k] =
                            maxvolch * (float) (vlayerch0[i][j][k]);
                        ...
                        for (isolid = 1; isolid <= nsolids; isolid++)
                        {
                            ...
                            //initial solids concentration (g/m3)
                            csedch[isolid][i][j][k] =
                                (float) (gsdch[isolid][i][j][k] * densityp *
                                         (1.0 -
                                          porositych[i][j][k]) * 1000.0);
    ...
    //
    //Allocate initial memory for overland solids (soil) loads
    nswovt = (float **) malloc ((nsolids + 1) * sizeof (float *));      //next (upper) interval time for sediment load time series
    pswovt = (float **) malloc ((nsolids + 1) * sizeof (float *));      //prior (lower) interval time for sediment load time series
    bswov = (float **) malloc ((nsolids + 1) * sizeof (float *));       //intercept for sediment load interpolation
    mswov = (float **) malloc ((nsolids + 1) * sizeof (float *));       //slope for sediment load interpolation
    swovpoint = (int **) malloc ((nsolids + 1) * sizeof (int *));       //pointer (to position in time series) for sediment loads
    swovinterp = (float **) malloc ((nsolids + 1) * sizeof (float *));  //external sediment load interpolated in time for solids type [i] source [j]
    ntswov = (float *) malloc ((nsolids + 1) * sizeof (float)); //next time to update any load function for solids type [i]
    ...
    for (i = 1; i <= nsolids; i++)
    {
        //Allocate remaining memory for overland solids (soil) loads
        nswovt[i] = (float *) malloc ((nswov[i] + 1) * sizeof (float)); //next (upper) interval time for sediment load time series
        pswovt[i] = (float *) malloc ((nswov[i] + 1) * sizeof (float)); //prior (lower) interval time for sediment load time series
        bswov[i] = (float *) malloc ((nswov[i] + 1) * sizeof (float));  //intercept for sediment load interpolation
        mswov[i] = (float *) malloc ((nswov[i] + 1) * sizeof (float));  //slope for sediment load interpolation
        swovpoint[i] = (int *) malloc ((nswov[i] + 1) * sizeof (int));  //pointer (to position in time series) for sediment loads
        swovinterp[i] = (float *) malloc ((nswov[i] + 1) * sizeof (float));     //external sediment load interpolated in time for source[i]
    ...
    //loop over number of soilds reports (include zero for sum of solids)
    for (i = 0; i <= nsgroups; i++)
    {
        ...
        if (sedexpfile_fp == NULL)
        {
            ...
            printf
                ("Solids Type = %d  Can't create Solids Export File: %s \n",
                 i, sedexpfile[i]);
    ...
    solidsdepositiongrid =
        (char **) malloc ((nsgroups + 1) * sizeof (char *));
    solidsnetaccumgrid = (char **) malloc ((nsgroups + 1) * sizeof (char *));

    //loop over number of soilds reports
    for (i = 0; i <= nsgroups; i++)
    {
        //Allocate remaining memory for solids grid files
        solidsconcwatergrid[i] =
        ...
        if (i == 0)
        {
            ...
            solidsdepositiongrid[i] =
                strcat (solidsdepositiongrid[i], "total");
        ...
        else                    //else the group number > 0
        {
            //Add reporting group name to grid file names
            solidsconcwatergrid[i] =
                strcat (solidsconcwatergrid[i], sgroupname[i]);
            solidsconcsurfgrid[i] =
                strcat (solidsconcsurfgrid[i], sgroupname[i]);
            solidserosiongrid[i] =
                strcat (solidserosiongrid[i], sgroupname[i]);
            solidsdepositiongrid[i] =
                strcat (solidsdepositiongrid[i], sgroupname[i]);
            solidsnetaccumgrid[i] =
                strcat (solidsnetaccumgrid[i], sgroupname[i]);
