...
void ChannelChemicalDissolution(void)
{
    ...
    float kd,                   //dissolution rate (m2/s)
      csol,                     //chemical solubility limit (g/m3)
      alpha;                    //surface area (of solids) for dissolution (m2)

    double potential,           //solids mass dissolution potential (g)
            ...
            for (ichem = 1; ichem <= nchems; ichem++)
            {
                ...
                if (dslopt[ichem] > 0)
                {
                    ...
                    for (iyield = 1; iyield <= ncyields; iyield++)
                    {
                        ...
                        if (cyldprocess[iyield] == 8
                            && cyldto[iyield] == ichem)
                        {
                            //set the reacting soilds type
                            ...
                            available =
                                csedchnew[isolid][i][j][0] * newwatervol;
                            ...
                            if (newsolidsmass > 0)
                            {
                                ...
                                csedchnew[isolid][i][j][0] =
                                    (float) (newsolidsmass / newwatervol);
                            ...
                            //compute surface area available for dissolution (m2)
                            alpha =
                                (float) ((6.0 * csedch[isolid][i][j][ilayer] *
                                          sedvol) / (ds[isolid] *
                                                     spgravity[isolid] *
                                                     densityw));
                            ...
                            //compute the dissolution potential (g)
                            potential =
                                dslsedchoutflux[isolid][i][j][ilayer] *
                                dt[idt];
                            ...
                            //compute the mass available for dissolution (g)
                            available =
                                csedchnew[isolid][i][j][ilayer] * newsedvol;
