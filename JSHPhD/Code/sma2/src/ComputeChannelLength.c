...
void ComputeChannelLength(void)
{
    //local variable declarations
    int samerow,                //nodal difference in row location
      samecol;                  //nodal difference in column location

    int downlink;               //downstream link at a junction
    ...
    for (i = 1; i <= nlinks; i++)
    {
        ...
        for (j = 1; j <= nnodes[i] - 1; j++)
        {
            ...
            {
                //Set cell half lengths: 0.5 * sqrt(2) * cell size
                lengthdown[i][j] = (float) (0.5 * sqrt (2) * w);        //downstream half length of current node
                lengthup[i][j + 1] = (float) (0.5 * sqrt (2) * w);      //upstream half length of downstream node
        ...
        else if (ndownbranches[i][0] == 1)      //there is exactly one downstream branch
        {
            ...
            {
                //Set cell half lengths: 0.5 * sqrt(2) * cell size
                lengthdown[i][nnodes[i]] = (float) (0.5 * sqrt (2) * w);        //downstream half length of current node
                lengthup[i][nnodes[i] + 1] = (float) (0.5 * sqrt (2) * w);      //upstream half length of downstream node
            }                   //end if samerow = 0 or samecol = 0
        }
        else                    //there is more than one downstream branch > 1
        {
            ...
            for (j = 1; j <= 8; j++)
            {
                ...
                if (ndownbranches[i][j] > 0)
                {
                    ...
                    {
                        //Set cell half lengths: 0.5 * sqrt(2) * cell size
                        lengthdown[downlink][0] = (float) (0.5 * sqrt (2) * w); //downstream half length of current node
                        lengthup[downlink][1] = (float) (0.5 * sqrt (2) * w);   //upstream half length of downstream node
                    }           //end if samerow = 0 or samecol = 0
    ...
    for (i = 1; i <= nlinks; i++)
    {
        ...
        for (j = 1; j <= nnodes[i] - 1; j++)
        {
            ...
            chanlength[i][j] =
                (lengthdown[i][j] + lengthup[i][j + 1]) * sinuosity[i][j];
        ...
        else if (ndownbranches[i][0] == 1)      //there is exactly one downstream branch
        {
            ...
            fprintf (echofile_fp, "   %11d %4d   %10d %4d   %12.3f\n",
                     i, j,
                     link[ichnrow[i][nnodes[i] + 1]][ichncol[i]
                                                     [nnodes[i] + 1]],
                     node[ichnrow[i][nnodes[i] + 1]][ichncol[i]
                                                     [nnodes[i] + 1]],
                     chanlength[i][j]);
        }
        else                    //there is more than one downstream branch > 1
            ...
            for (j = 1; j <= 8; j++)
            {
                ...
                if (ndownbranches[i][j] > 0)
                {
                    ...
                    chanlength[downlink][0] = (lengthdown[downlink][0] +
                                               lengthup[downlink][1]) *
                        sinuosity[i][nnodes[i]];

                    //Echo channel lengths to file
                    fprintf (echofile_fp, "   %11d %4d   %10d %4d   %12.3f\n",
                             i, nnodes[i], downlink, 1,
                             chanlength[downlink][0]);
