...
void ChannelChemicalErosion(void)
{
    //local variable declarations/definitions
    int ichem,                  //loop index for chemicals
      isolid,                   //loop index for solids (particle) type
      ilayer;                   //index for surface layer in the sediment stack

    float sumfp,                //sum of particulate fractions
      fmobile;                  //sum of mobile fractions (dissolved + bound)

    double potential,           //chemical erosion mass potential (g)
        ...
        for (j = 1; j <= nnodes[i]; j++)
        {
            ...
            for (ichem = 1; ichem <= nchems; ichem++)
            {
                ...
                for (isolid = 1; isolid <= nsolids; isolid++)
                {
                    ...
                    erschemchoutflux[ichem][i][j][ilayer] =
                        erschemchoutflux[ichem][i][j][ilayer] +
                    ...
                    sumfp =
                        sumfp + fparticulatech[ichem][isolid][i][j][ilayer];
                ...
                available =
                    vlayerch[i][j][ilayer] * cchemch[ichem][i][j][ilayer] *
                    sumfp - (biochemchoutflux[ichem][i][j][ilayer] +
                ...
                if (bulkvolume > 0.0)
                {
                    ...
                    pwrchemchoutflux[ichem][i][j][ilayer] =
                        (float) (bulkvolume) * cchemch[ichem][i][j][ilayer] *
                        fmobile / dt[idt];
                }
                ...
                available =
                    vlayerch[i][j][ilayer] * cchemch[ichem][i][j][ilayer] *
                    fmobile - (biochemchoutflux[ichem][i][j][ilayer] +
                               hydchemchoutflux[ichem][i][j][ilayer] +
                               oxichemchoutflux[ichem][i][j][ilayer] +
                               phtchemchoutflux[ichem][i][j][ilayer] +
                               radchemchoutflux[ichem][i][j][ilayer] +
