...
void OverlandSolidsTransportCapacity(void)
{
    //local variable declarations/definitions
    int isoil,                  //local index for soil type
    ...
    float sf,                   //friction slope (dimensionless)
      ...
      tbs,                      //total bed solids concentration (sum of all solids in surface layer) (g/m3)
        ...
        for (j = 1; j <= ncols; j++)
        {
            ...
            if (imask[i][j] != nodatavalue)
            {
                ...
                for (k = 1; k <= 8; k++)
                {
                    ...
                    if (q > qc)
                    {
                        //total (all soilds) overland transport capacity (kg/m/s)
                ...
                //
                //compute total bed solids (tbs) and transport fraction...
                //
                //initialize total bed solids for the surface layer
                tbs = 0.0;
                ...
                {
                    //compute total bed solids for the surface layer (g/m3)
                    tbs = tbs + csedov[isolid][i][j][ilayer];
                ...
                for (isolid = 1; isolid <= nsolids; isolid++)
                {
                    ...
                    if (tbs > 0.0)
                    {
                        ...
                        fsolids = csedov[isolid][i][j][ilayer] / tbs;
                    }
                    else        //else tbs = 0
                    {
