...
C-              values at specific times.  Forcing functions for rain,
...
C-
C-
C-	Inputs:	    rfintensity[][], qwov[][], qwch[][], hbc[][]
C-              (at some time t in time series)
C-
C-	Outputs:    rfinterp[][], qwovinterp[][], qwchinterp[][],
C-              (at current simulation time)
C-
C-	Controls:   chnopt, nrg, nrpairs, nqwov, nqwovpairs, nqwch, nqwchpairs,
C-              noutlets, dbcopt, nqbcpairs
C-
C-	Calls:      None
C-
...
void UpdateTimeFunctionWater(void)
{
    ...
    if (nqwov > 0)
    {
        ...
        if (simtime >= ntqwov)
        {
            ...
            for (i = 1; i <= nqwov; i++)
            {
                ...
                if (simtime >= nqwovt[i] || simtime < pqwovt[i])
                {
                    ...
                    nqwovt[i] =
                        (float) (ncycle * endtime) + qwovtime[i][ip + 1];
        ...
        for (i = 1; i <= nqwov; i++)
        {
            ...
            qwovinterp[i] =
                (float) (mqwov[i] * (simtime - nqwovt[i]) + bqwov[i]);
    ...
    if (chnopt > 0)
    {
        ...
        if (simtime >= ntqwch)
        {
            ...
            for (i = 1; i <= nqwch; i++)
            {
                ...
                if (simtime >= nqwcht[i] || simtime < pqwcht[i])
                {
                    ...
                    //Next time interval upper bounds to recalculate this external flow BC.
                    nqwcht[i] =
                        (float) (ncycle * endtime) + qwchtime[i][ip + 1];
        ...
        for (i = 1; i <= nqwch; i++)
        {
            ...
            qwchinterp[i] =
                (float) (mqwch[i] * (simtime - nqwcht[i]) + bqwch[i]);
        ...
        for (i = 1; i <= noutlets; i++)
        {
            ...
            if (dbcopt[i] > 0)
            {
                ...
                if (simtime >= nhbct[i] || simtime < phbct[i])
                {
                    ...
                    nhbct[i] =
                        (float) (ncycle * endtime) + hbctime[i][ip + 1];
