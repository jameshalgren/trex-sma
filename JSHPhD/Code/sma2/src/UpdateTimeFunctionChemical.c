...
C-
C-
C-	Inputs:		cwov[][][], cwch[][][], cbc[][][],
C-              (at some time t in time series)
C-
C-	Outputs:	cwovinterp[][][], cwchinterp[][][], cbcinterp[][][]
C-              (at current simulation time)
C-
C-	Controls:	chnopt, nchems, ncwov, ncwovpairs, ncwch, ncwchpairs,
C-              noutlets, dbcopt, ncbcpairs
C-
C-	Calls:		None
C-
...
void UpdateTimeFunctionChemical(void)
{
    ...
    for (i = 1; i <= nchems; i++)
    {
        //If it is time to update any overland chemical load time function (simtime >= ntcwov[])
        if (simtime >= ntcwov[i])
        {
            ...
            //Update overland chemical loads...
            //
            //Loop over number of overland oads for the current chemical
            for (j = 1; j <= ncwov[i]; j++)
            {
                //if it is time to update values for this chemical load...
                //
                //ncwovt = next time value in overland chemical load time series
                //pcwovt = prior overland chemical load time value in time series
                //
                if (simtime >= ncwovt[i][j] || simtime < pcwovt[i][j])
                {
                    ...
                    endtime = cwovtime[i][j][ncwovpairs[i][j]];
                    ...
                    while (mtime >= cwovtime[i][j][ip + 1])
                    {
                        ip = ip + 1;

                    }           //end while

                    while ((mtime < cwovtime[i][j][ip]) && (ip != 1))
                    {
                        ip = ip - 1;
                    ...
                    bcwov[i][j] = cwov[i][j][ip + 1];

                    //Compute slope (m) for new interval
                    mcwov[i][j] = (cwov[i][j][ip] - cwov[i][j][ip + 1])
                        / (cwovtime[i][j][ip] - cwovtime[i][j][ip + 1]);
                    ...
                    ncwovt[i][j] =
                        (float) (ncycle * endtime) + cwovtime[i][j][ip + 1];

                    //Next time interval lower bounds to recalculate this chemical load.
                    pcwovt[i][j] =
                        (float) (ncycle * endtime) + cwovtime[i][j][ip];
        ...
        for (j = 1; j <= ncwov[i]; j++)
        {
            ...
            cwovinterp[i][j] =
                (float) (mcwov[i][j] * (simtime - ncwovt[i][j]) +
                         bcwov[i][j]);
    ...
    if (chnopt > 0)
    {
        ...
        for (i = 1; i <= nchems; i++)
        {
            ...
            if (simtime >= ntcwch[i])
            {
                ...
                for (j = 1; j <= ncwch[i]; j++)
                {
                    ...
                    if (simtime >= ncwcht[i][j] || simtime < pcwcht[i][j])
                    {
                        ...
                        //Next time interval upper bounds to recalculate this chemical load.
                        ncwcht[i][j] =
                            (float) (ncycle * endtime) + cwchtime[i][j][ip +
                                                                        1];
            ...
            for (j = 1; j <= ncwch[i]; j++)
            {
                //Compute linearly interpolated external channel flows for this time step
                cwchinterp[i][j] =
                    (float) (mcwch[i][j] * (simtime - ncwcht[i][j]) +
                             bcwch[i][j]);
    ...
    for (i = 1; i <= noutlets; i++)
    {
        ...
        if (dbcopt[i] > 0)
        {
            ...
            if (simtime >= ntcbc[i])
            {
                ...
                for (j = 1; j <= nchems; j++)
                {
                    ...
                    if (simtime >= ncbct[i][j] || simtime < pcbct[i][j])
                    {
                        ...
                        ncbct[i][j] =
                            (float) (ncycle * endtime) + cbctime[i][j][ip +
                                                                       1];
            ...
            for (j = 1; j <= nsolids; j++)
            {
                ...
                cbcinterp[i][j] =
                    (float) (mcbc[i][j] * (simtime - ncbct[i][j]) +
                             bcbc[i][j]);
