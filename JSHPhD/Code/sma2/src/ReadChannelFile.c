...
void ReadChannelFile(void)
{
    //local variable declarations
    int chanlinks,              //number of channel links
    ...
    {
        printf ("Can't open Channel File: %s \n", channelfile); //Write message to screen
    ...
    for (i = 1; i <= nlinks; i++)
    {
        ...
        if (channodes != nnodes[i])
        {
            ...
            fprintf (echofile_fp, "\n\n\nChannel File Error:\n");
            fprintf (echofile_fp,
                     "  link = %5d   channodes = %5d   nnodes = %5d\n", i,
                     channodes, nnodes[i]);
        ...
        for (j = 1; j <= nnodes[i]; j++)
        {
            ...
            fprintf (echofile_fp,
                     "%4d %5d %11.3f %16.3f %10.3f %9.4f %10.3f %16.3f\n", i,
                     j, bwidth[i][j], sideslope[i][j], hbank[i][j],
                     nmanningch[i][j], sinuosity[i][j],
                     deadstoragedepth[i][j]);

            //Note:  The channel top width at the top of the bank is constant
            //       and is used as a control to 1) ensure the channel width
            //       is less than 80% of the cell width, and 2) recompute the
            //       channel side slope when aggradation/degradation occurs.
            //
            //Compute channel top width at top of bank (m)
            twidth[i][j] =
                (float) (bwidth[i][j] + 2.0 * sideslope[i][j] * hbank[i][j]);

            //Two more error checks...
            //
            //The channel top width must be < 80% of the cell width.
            //
            //If top width > 0.8 * w, abort...
            if (twidth[i][j] > (float) (0.80 * w))
            {
                //Write error message to file
                fprintf (echofile_fp, "\n\n\nChannel File Error:\n");
                fprintf (echofile_fp,
                         "  Top width exceeds 80%% of cell width...\n");
                fprintf (echofile_fp, "  link = %d  node = %d\n", i, j);
                fprintf (echofile_fp, "  top width = %f  cell width = %f\n",
                         twidth[i][j], w);
                fprintf (echofile_fp, "/n  top width must be less than: %f\n",
                         (float) (0.8 * w));

                //Write error message to screen
                printf ("Channel File Error:\n");
                printf ("  Top width exceeds 80%% of cell width...\n");
                printf ("  link = %d  node = %d\n", i, j);
                printf ("  top width = %f  cell width = %f\n",
                        twidth[i][j], w);
                printf ("/n  top width must be less than: %f\n",
                        (float) (0.8 * w));
            ...
            if (twidth[i][j] < bwidth[i][j])
            {
                ...
                fprintf (echofile_fp, "\n\n\nChannel File Error:\n");
                fprintf (echofile_fp,
                         "  Top width less than bottom width...\n");
                fprintf (echofile_fp, "  link = %d  node = %d\n", i, j);
