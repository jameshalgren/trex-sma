...
void WriteTimeSeriesChemical(void)
{
    ...
    float **totcgroupsumov,     //reporting group sum for total chemical in the overland plane [igroup][ireport]
    ...
    if (nchemreports > 0)
    {
        ...
        for (igroup = 1; igroup <= ncgroups; igroup++)
        {
            ...
            if (chemexpfile_fp == NULL)
            {
                ...
                printf ("Can't open Chemical Export File:  %s\n",
                        chemexpfile[igroup]);
            ...
            for (j = 1; j <= nchemreports; j++)
            {
                ...
                if (chemunitsopt[j] == 2)
                {
                    ...
                    for (k = 1; k <= 8; k++)
                    {
                        //compute sum of outflows (m3/s)
                        dqsum =
                            dqsum + dqovout[sedreprow[j]][sedrepcol[j]][k];
                    ...
                    fprintf (chemexpfile_fp, ",%e",
                             discgroupsumov[igroup][j] * dischargeov *
                             chemconvert[j]);
                    ...
                    fprintf (chemexpfile_fp, ",%e",
                             bndcgroupsumov[igroup][j] * dischargeov *
                             chemconvert[j]);
                    ...
                    fprintf (chemexpfile_fp, ",%e",
                             prtcgroupsumov[igroup][j] * dischargeov *
                             chemconvert[j]);
