...
void FreeMemoryEnvironment(void)
{
    ...
    //Deallocate memory for environmental property file names
    free (envpropfile);         //deallocate memory
    ...
    for (i = 1; i <= nrows; i++)
    {
        ...
        for (j = 1; j <= ncols; j++)
        {
            ...
            free (watertempov[i][j]);   //overland water temperature (C) [row][col][layer]
        ...
        free (watertempov[i]);  //overland water temperature (C) [row][col][layer]
    ...
    free (watertempov);         //overland water temperature (C) [row][col][layer]
    ...
    if (chnopt > 0)
    {
        ...
        for (i = 1; i <= nlinks; i++)
        {
            ...
            for (j = 1; j <= nnodes[i]; j++)
            {
                ...
                free (watertempch[i][j]);       //channel water temperature (C) [link][node][layer]
            ...
            free (watertempch[i]);      //channel water temperature (C) [link][node][layer]
        ...
        free (watertempch);     //channel water temperature (C) [link][node][layer]
