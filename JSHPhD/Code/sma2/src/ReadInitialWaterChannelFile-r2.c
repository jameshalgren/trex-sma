...
void ReadInitialWaterChannelFile(void)
{
    ...
    {
        printf ("Can't open Initial Water Depth Channel File : %s \n", waterchannelfile);       //Write message to screen
    ...
    for (i = 1; i <= nlinks; i++)
    {
        ...
        for (j = 1; j <= nnodes[i]; j++)
        {
            ...
            fprintf (echofile_fp, "%4d  %6d  %25.5f", i, j, hch[i][j]);
