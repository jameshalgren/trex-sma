...
//mlv           free(initialchemchfile);        //deallocate memory
    ...
    for (i = 1; i <= nchems; i++)
    {
        ...
        if (ncwov[i] > 0)
        {
            ...
            for (j = 1; j <= ncwov[i]; j++)
            {
                ...
                free (cwov[i][j]);      //deallocate memory
                free (cwovtime[i][j]);  //deallocate memory

            }                   //end loop over number of overland loads

            //Deallocate memory for overland chemical loads/forcing functions
            free (cwovrow[i]);  //deallocate memory
            free (cwovcol[i]);  //deallocate memory
            free (ncwovpairs[i]);       //deallocate memory
            free (cwovopt[i]);  //deallocate memory
            free (cwov[i]);     //deallocate memory
            free (cwovtime[i]); //deallocate memory

        }                       //end if ncwov > 0

    }                           //end loop over chemicals

    //Deallocate memory for overland chemical loads/forcing functions (w)
    free (ncwov);               //deallocate memory
    free (cwovrow);             //deallocate memory
    free (cwovcol);             //deallocate memory
    free (ncwovpairs);          //deallocate memory
    free (cwovopt);             //deallocate memory
    free (cwov);                //deallocate memory
    free (cwovtime);            //deallocate memory
    ...
    for (i = 1; i <= nchems; i++)
    {
        ...
        free (ncwovt[i]);       //next (upper) interval time for chemical load time series
        free (pcwovt[i]);       //prior (lower) interval time for chemical load time series
        free (bcwov[i]);        //intercept for chemical load interpolation
        free (mcwov[i]);        //slope for chemical load interpolation
        free (cwovpoint[i]);    //pointer (to position in time series) for chemical loads
        free (cwovinterp[i]);   //external chemical load interpolated in time for source[i]
    ...
    //Deallocate memory for chemical loads
    free (ncwovt);              //next (upper) interval time for chemical load time series
    free (pcwovt);              //prior (lower) interval time for chemical load time series
    free (bcwov);               //intercept for chemical load interpolation
    free (mcwov);               //slope for chemical load interpolation
    free (cwovpoint);           //pointer (to position in time series) for chemical loads
    free (cwovinterp);          //external chemical load interpolated in time for source[i]
    free (ntcwov);              //next time to update any load function for chemical type [i]
