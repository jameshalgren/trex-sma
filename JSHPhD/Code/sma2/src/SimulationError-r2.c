...
C-
C-              simerrorno = 1: Negative depth in overland cell
C-
C-              simerrorno = 2: Negative depth in channel cell
C-
C-              simerrorno = 3: Negative solids concentration in overland cell
C-
C-              simerrorno = 4: Negative solids concentration in channel cell
C-
C-              simerrorno = 5: Negative chemical concentration in overland cell
C-
C-              simerrorno = 6: Negative chemical concentration in channel cell
C-
C-
...
void SimulationError(int simerrorno, int ierr, int jerr, int itype)
{
    ...
    printf
        ("     Please read the Simulation Error File for error message details...\n\n");
    ...
    //negative depth in overland cell
    if (simerrorno == 1)
    {
        ...
        //Write location of negative depth in overland cell
        fprintf (errorfile_fp,
                 "Overland cell row: %d  column: %d  imask = %d\n", ierr,
                 jerr, imask[ierr][jerr]);

        //Write current value of depth in overland cell (row/col)
        fprintf (errorfile_fp,
                 "Ending water depth in this cell is: %f m.\n",
                 hovnew[ierr][jerr]);

        //Write prior value of depth in overland cell (row/col)
        fprintf (errorfile_fp,
                 "Starting water depth in this cell is: %f m.\n",
                 hov[ierr][jerr]);
        ...
        if (imask[ierr][jerr] > 1)
        {
            ...
            fprintf (errorfile_fp,
                     "Overland/Channel Cell link: %d  node: %d  imask = %d\n",
                     link[ierr][jerr], node[ierr][jerr], imask[ierr][jerr]);

            //Write current value of depth in channel (link/node)
            fprintf (errorfile_fp,
                     "Ending water depth in this link/node is: %f m.\n",
                     hchnew[link[ierr][jerr]][node[ierr][jerr]]);

            //Write prior value of depth in channel (link/node)
            fprintf (errorfile_fp,
                     "Starting water depth in this link/node is: %f m.\n",
                     hch[link[ierr][jerr]][node[ierr][jerr]]);

            //Write channel bank height (link/node)
            fprintf (errorfile_fp,
                     "Channel bank height in this link/node is: %f m.\n",
    ...
    //negative depth in channel
    else if (simerrorno == 2)
    {
        //Write general simulation running time error message
        fprintf (errorfile_fp,
                 "TREX stopped running during time loop; input file is: %s \n\n",
        ...
        //Write error type
        fprintf (errorfile_fp,
                 "A negative depth has occurred in a channel link/node\n");

        //Write location of negative depth in channel (link/node)
        ...
        //Write current value of depth in overland cell (row/col)
        fprintf (errorfile_fp,
                 "Ending overland water depth in this cell is: %f m.\n",
                 hovnew[ichnrow[ierr][jerr]][ichncol[ierr][jerr]]);

        //Write prior value of depth in overland cell (row/col)
        fprintf (errorfile_fp,
                 "Starting overland water depth in this cell is: %f m.\n",
    ...
    else if (simerrorno == 3)
    {
        ...
        fprintf (errorfile_fp,
                 "Overland row: %d  col: %d  isolid: %d  imask: %d\n", ierr,
                 jerr, itype, imask[ierr][jerr]);

        //Write current value of negative solids concentration overland (row/col)
        fprintf (errorfile_fp,
                 "Ending solids concentration in this row/col is: %f g/m3.\n",
                 csedovnew[itype][ierr][jerr][0]);

        //Write prior value of negative solids concentration overland (row/col)
        fprintf (errorfile_fp,
                 "Starting solids concentration in this row/col is: %f g/m3.\n",
    ...
    else if (simerrorno == 4)
    {
        ...
        fprintf (errorfile_fp,
                 "Ending solids concentration in this link/node is: %f g/m3.\n",
                 csedchnew[itype][ierr][jerr][0]);

        //Write prior value of concentration in channel (link/node)
        fprintf (errorfile_fp,
                 "Starting solids concentration in this link/node is: %f g/m3.\n",
    ...
    else if (simerrorno == 6)
    {
        ...
        //Write current value of concentration in channel (link/node)
        fprintf (errorfile_fp,
                 "Ending chemical concentration in this link/node is: %f g/m3.\n",
                 cchemchnew[itype][ierr][jerr][0]);

        //Write prior value of concentration in channel (link/node)
        fprintf (errorfile_fp,
                 "Starting chemical concentration in this link/node is: %f g/m3.\n",
    ...
    else if (simerrorno == 7)
    {
        ...
        fprintf (errorfile_fp,
                 "Ending bank height in this row/col is: %f m.\n",
                 hbank[link[ierr][jerr]][node[ierr][jerr]]);
    ...
    else if (simerrorno == 8)
    {
        ...
        //Write location of depositing chanel element (link/node)
        fprintf (errorfile_fp, "Channel link: %d node: %d  nstackch: %d\n",
                 ierr, jerr, itype);

        //Write new value of bank height (link/node)
        fprintf (errorfile_fp,
                 "Ending bank height in this link/node is: %f m.\n",
                 hbank[ierr][jerr]);
    }
    ...
    else if (simerrorno == 9)
    {
        ...
        fprintf (errorfile_fp,
                 "  stkovopt = 1 or increase maxstackov...\n\n");
    ...
    else if (simerrorno == 10)
    {
        ...
        fprintf (errorfile_fp, "  Exceeded maximum number of layers. Set\n");
        fprintf (errorfile_fp,
                 "  stkchopt = 1 or increase maxstackch...\n\n");
