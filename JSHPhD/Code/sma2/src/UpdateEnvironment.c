...
void UpdateEnvironment(void)
{
    ...
    int pid,                    //index for property identification
                ...
                for (j = 1; j <= ncols; j++)
                {
                    ...
                    if (imask[i][j] > 0)
                    {
                        ...
                        if (tfid > 0)
                        {
                            ...
                            windspeed[i][j] =
                                envg[iprop][i][j] * envgtfinterp[iprop][tfid];
                        }
                ...
                for (j = 1; j <= ncols; j++)
                {
                    ...
                    if (imask[i][j] > 0)
                    {
                        ...
                        for (k = 0; k <= maxstackov; k++)
                        {
                            ...
                            if (tfid > 0)
                            {
                                //set the water temperature value (C) f(x,y,z,t)
                                watertempov[i][j][k] =
                                    envov[iprop][i][j][k] *
                                    envovtfinterp[iprop][tfid];
                            }
                            ...
                            {
                                //set the water temperature value (C) f(x,y,z)
                                watertempov[i][j][k] = envov[iprop][i][j][k];
                ...
                for (i = 1; i <= nlinks; i++)
                {
                    ...
                    for (j = 1; j <= nnodes[i]; j++)
                    {
                        ...
                        for (k = 0; k <= maxstackch; k++)
                        {
                            ...
                            {
                                //set the water temperature value (C) f(x,y,z)
                                watertempch[i][j][k] = envch[iprop][i][j][k];
