...
void FloodplainWaterTransfer(void)
{
    ...
    float hcell,                //effective overland cell water depth (m)
    ...
    float vtransfer;            //volume of water transferred between overland plane/channel (m3)

    float wseov,                //water surface elevation (wse) of the overland plane (m)
    ...
    if (fldopt > 0)
    {
        ...
        for (i = 1; i <= nlinks; i++)
        {
            ...
            for (j = 1; j <= nnodes[i]; j++)
            {
                ...
                qnode =
                    (dqchout[i][j][1] + dqchout[i][j][2] + dqchout[i][j][3] +
                ...
                //exist depending on water suface elevation (wse) and whether the
                ...
                if ((hcell > scell) && (wseov > wsech))
                {
                    ...
                    if (hnode < hbch)
                    {
                        ...
                        vcell = (float) ((hcell - scell) * (w * w - achsurf));
                    ...
                    dqchinvol[i][j][9] = dqchinvol[i][j][9] + vtransfer;
                ...
                else if (wsech > wseov)
                {
                    ...
                    dqchoutvol[i][j][9] = dqchoutvol[i][j][9] + vtransfer;
