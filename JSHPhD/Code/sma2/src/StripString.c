...
char * StripString (char *s)
{
    ...
    int offset;                 //offset position in character string array

    //size_t is a data type for holding sizes returned by the strlen() function
    size_t len;                 //length of the character string (after leading spaces removed)
    ...
    while ((*(s + len - offset) == ' ' || *(s + len - offset) == '\t')
           && (*s != '\n'))
    {
