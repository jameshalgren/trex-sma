...
void TimeFunctionInitEnvironment(void)
{
    ...
    for (i = 1; i <= npropg; i++)
    {
        ...
        for (j = 1; j <= nenvgtf[i]; j++)
        {
            ...
            while ((simtime > envgtftime[i][j][ip])
                   && (ip < nenvgtfpairs[i][j]))
            {
    ...
    for (i = 1; i <= npropov; i++)
    {
        ...
        for (j = 1; j <= nenvovtf[i]; j++)
        {
            ...
            //  time of the current solids load AND the time series
            //  index is less than the number of pairs input
            while ((simtime > envovtftime[i][j][ip])
                   && (ip < nenvovtfpairs[i][j]))
            {
                //increment the time series position index
                ip = ip + 1;
    ...
    if (chnopt > 0)
    {
        ...
        for (i = 1; i <= npropch; i++)
        {
            ...
            for (j = 1; j <= nenvchtf[i]; j++)
            {
                ...
                //  time of the current solids load AND the time series
                //  index is less than the number of pairs input
                while ((simtime > envchtftime[i][j][ip])
                       && (ip < nenvchtfpairs[i][j]))
                {
                    //increment the time series position index
                    ip = ip + 1;
    ...
    if (fpocovopt > 0)
    {
        ...
        for (i = 1; i <= nsolids; i++)
        {
            ...
            for (j = 1; j <= nfpocovtf[i]; j++)
            {
                ...
                //  time of the current solids load AND the time series
                //  index is less than the number of pairs input
                while ((simtime > fpocovtftime[i][j][ip])
                       && (ip < nfpocovtfpairs[i][j]))
                {
                    //increment the time series position index
                    ip = ip + 1;
        ...
        if (fpocchopt > 0)
        {
            ...
            for (i = 1; i <= nsolids; i++)
            {
                ...
                for (j = 1; j <= nfpocchtf[i]; j++)
                {
                    ...
                    //  time of the current solids load AND the time series
                    //  index is less than the number of pairs input
                    while ((simtime > fpocchtftime[i][j][ip])
                           && (ip < nfpocchtfpairs[i][j]))
                    {
                        //increment the time series position index
                        ip = ip + 1;
