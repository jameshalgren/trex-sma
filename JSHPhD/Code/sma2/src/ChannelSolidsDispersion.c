...
void ChannelSolidsDispersion(void)
{
    ...
    float hchan,                //depth of water in present node (m)
    ...
    float cadj[11],             //sediment concentration of adjacent cell/node (g/m3)
    ...
    for (i = 1; i <= nlinks; i++)
    {
        ...
        for (j = 1; j <= nnodes[i]; j++)
        {
            ...
            if (hchan <= hbch)
            {
                ...
                wp = (float) (bwch +
                              2.0 * hchan * sqrt (1.0 + sslope * sslope));
            }
                ...
                if (j == 1)
                {
                    ...
                    for (k = 1; k <= 8; k++)
                    {
                        ...
                        if (nupbranches[i][k] > -1)
                        {
                            ...
                            if (hch[adjlink][adjnode] <=
                                hbank[adjlink][adjnode])
                            {
                                //set the cross sectional area of the adjacent node (m2)
                                achadj =
                                    (bwidth[adjlink][adjnode] +
                                     sideslope[adjlink][adjnode] *
                                     hch[adjlink][adjnode]) *
                                    hch[adjlink][adjnode];
                            }
                            else        //else water depth > bank height
                    ...
                    for (k = 1; k <= 8; k++)
                    {
                        ...
                        if (k == updirection[i][j][0])
                        {
                            ...
                            else        //else water depth > bank height
                            {
                                ...
                                achadj =
                                    (bwidth[i][j - 1] +
                                     sideslope[i][j - 1] * hbank[i][j -
                                                                    1]) *
                                    hbank[i][j - 1] + twch * (hch[i][j - 1] -
                                                              hbank[i][j -
                                                                       1]);
                ...
                for (k = 1; k <= 9; k++)
                {
                    ...
                    if (cgrad > 0.0)
                    {
                        ...
                        dspsedchoutmass[isolid][i][j][k] =
                            dspsedchoutmass[isolid][i][j][k] +
                            (float) (dspsedchoutflux[isolid][i][j][k] *
                                     dt[idt] / 1000.0);
                    }
                    ...
                    else if (cgrad < 0.0)
                    {
                        ...
                        dspsedchinmass[isolid][i][j][k] =
                            dspsedchinmass[isolid][i][j][k] +
                            (float) (dspsedchinflux[isolid][i][j][k] *
                                     dt[idt] / 1000.0);
                    }
