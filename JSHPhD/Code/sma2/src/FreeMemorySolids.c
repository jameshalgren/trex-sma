...
//mlv           free(initialsschfile);
    ...
    for (i = 1; i <= nsolids; i++)
    {
        ...
        if (nswov[i] > 0)
        {
            ...
            //Free memory for overland solids loads/forcing functions
            free (swovrow[i]);
            free (swovcol[i]);
            free (nswovpairs[i]);
            free (swovopt[i]);
    ...
    //Free remaining memory for overland solids loads/forcing functions (w)
    free (nswov);
    free (swovrow);
    free (swovcol);
    free (nswovpairs);
...
//mlv   free(nstackov);
    ...
    for (i = 1; i <= nsolids; i++)
    {
        //Free memory for overland solids (soil) loads
        free (nswovt[i]);
        free (pswovt[i]);
        free (bswov[i]);
        free (mswov[i]);
