...
void ReadChannelTransmissionFile(void)
{
    ...
    {
        printf ("Can't open Channel Transmission Loss Property File: %s \n", channeltlossfile); //Write message to screen
    ...
    for (i = 1; i <= nlinks; i++)
    {
        ...
        for (j = 1; j <= nnodes[i]; j++)
        {
            ...
            fprintf (echofile_fp, "%4d %5d %29.12f %27.8f %30.8f\n",
                     i, j, khsed[i][j], capshsed[i][j], sedmd[i][j]);
