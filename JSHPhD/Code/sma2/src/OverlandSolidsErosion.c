...
void OverlandSolidsErosion(void)
{
    ...
    float sf,                   //friction slope (dimensionless)
    ...
    float epsilon,              //erosion amount (kg/m2)
    ...
    double potential,           //erosion mass potential (g)
        ...
        for (j = 1; j <= ncols; j++)
        {
            ...
            if (imask[i][j] != nodatavalue)
            {
                ...
                if (ersovopt > 1)
                {
                    ...
                    sf = (float) sqrt ((pow (sfov[i][j][1], 2)  //North component
                                        + pow (sfov[i][j][3], 2)        //East component
                                        + pow (sfov[i][j][5], 2)        //South component
                                        + pow (sfov[i][j][7], 2)));     //West component
                    ...
                    if (ersovopt > 1)
                    {
                        ...
                        if (cncopt[isolid] == 0)
                        {
                            ...
                            if (tau > tceov[isolid])
                            {
                                ...
                                epsilon =
                                    (float) (ayov[iland] / zageov[isolid] *
                                             pow ((tau / tceov[isolid] - 1.0),
                                                  mexpov[isoil]));
                            }
                            else        //else tau <= tceov
                            {
...
//mlv: inconsistent: we loop over 9, 10 here but not above or in the trans cap module...
                        ...
                        //compute the transport rate (g/s)
                        transrateov[isolid][i][j] = outflux
                            - depsedovoutflux[isolid][i][j][0];

                        //Note:  At this point the transport outfluxes
                        //       are not scaled to the available mass so
                        //       the computed transport rate can be less
                    ...
                    ersflowov[isolid][i][j] =
                        ersflowov[isolid][i][j] * ersovscale;
                    ...
                    available =
                        vlayerov[i][j][ilayer] * csedov[isolid][i][j][ilayer];
