...
void ChannelSolidsTransportCapacity(void)
{
        ...
        for (j = 1; j <= nnodes[i]; j++)
        {
            ...
            if (hchan > 0.0)
            {
                ...
                else            //Flow depth greater than channel depth
                {
                    ...
                    achcross =
                        (bwch + sslope * hbch) * hbch + (hchan - hbch) * twch;

                    //Wetted perimeter for trapezoidal, triangular, or rectangular channel
                    //plus rectangular perimeter above bank height
                    wp = (float) (bwch +
                                  2.0 * hbch * sqrt (1.0 + sslope * sslope) +
                                  2.0 * (hchan - hbch));
            ...
            for (isolid = 1; isolid <= nsolids; isolid++)
            {
                ...
                if (velocity > vcch[isolid])
                {
                    //Concentration by weight (weight sediment / weight total)
                    cw = (float) (0.05 *
                                  (spgravity[isolid] /
                                   (spgravity[isolid] - 1.0)) * ((velocity -
                                                                  vc) * sf) /
                                  sqrt ((spgravity[isolid] -
                                         1.0) * g * ds[isolid]) * sqrt ((rh *
                                                                         sf) /
                                                                        ((spgravity[isolid] - 1.0) * ds[isolid])));
                }
                else            //else velcoty <= vcch[]
                {
                    //set the concentration (transport capacity) to zero
                    cw = 0.0;

                }               //end if veleocity > vcch[]

                //Convert from Cw to Cg/m3 (g/m3) (Julien, 1998; p. 174)
                cgm3 = (float) ((1000000.0 * spgravity[isolid] * cw)
                                / (spgravity[isolid] +
                                   (1.0 - spgravity[isolid]) * cw));
